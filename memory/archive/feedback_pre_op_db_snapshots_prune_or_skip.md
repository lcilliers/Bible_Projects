...
- **Two scripts now fixed (both patched 2026-07-04):** `_apply_file_chapter_lexical_prose_v1_20260702.py` (pre-chapprose) **and** `_apply_backfill_chapter_verses_v1_20260702.py` (pre-backfill *and* pre-repair) each have **`--no-backup`** + a shared `snapshot_db(tag, prune_keep=…)` helper that **self-prunes the tag's snapshot family to the newest 2–3**. The backfill script was the bigger offender — it snapshots per-chapter and, unpatched, left **134 pre-backfill copies (~90GB)** after the Jeremiah→Ezekiel→Daniel run (recurrence caught 2026-07-04; only the chapprose script had been patched the first time). For any *other* loop-invoked `_apply_*` script, pass its skip-backup flag or prune its snapshot family afterward. When adding pre-op snapshots to a reusable script, **always** pair with a keep-N prune + a `--no-backup` opt-out.
