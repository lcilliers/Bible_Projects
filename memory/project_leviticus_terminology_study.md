...
**Researcher's driving questions (ride on top of the lexical study, extensible):** why necessary to clean · where "unclean" comes from · why COVER not scrub · IB-desire vs external vs prerequisite · does awareness come in · past-only or forward. Design principle: **each question = a filter/cross-tab over CODED indicator-dimensions** (not a re-read).
...
**Infrastructure (built + proven):** `scripts/_apply_lev_study_v1_20260705.py` — subcommands `--seed-questions`, `--load-coding <json>`, `--load-findings <json>`, `--live`; idempotent by provenance; keying + span-anchor fallback. Coding/finding JSONs in `verse-analysis/leviticus/_coding/` and `_discoveries/`. Design docs in `verse-analysis/leviticus/_reports/` (verse-records listing, terminology-orientation, coding-schema, corpus-datamodel). **Pilot done:** Lev 11:24-28 carcass-contact -> 108 ve_lexical rows (0 miss) + 1 evidenced finding; the `reset x source_domain` cross-tab already answers "cover vs scrub" (bodily/dietary = wait-evening/launder, NEVER cover).
