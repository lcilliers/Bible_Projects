# Psa 38 — Phase-1 per-verse lexical (DB-derived draft) — 2026-07-02

> Chapter-driven poetic build; within-verse items only; role = sanity-check DRAFT. 126 spans, 533 rows.


## Psa 38:1

- **Psalm** `H4210` [1/M22] **role=standalone** — sense=Psalm; type=status
- **David** `H1732` [2] **role=process-qualifier** — sense=David; type=status; operation=(qualifies) memorial offering; manner=manner-of memorial offering; coupling=welds memorial offering
- **memorial offering** `H2142` [1/M41] **role=characteristic** — sense=memorial offering; type=action; operation=memorial offering; bearer=David; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **Lord** `H3068` [2] **role=standalone** — sense=Lord; type=status; bearer=David; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **rebuke** `H3198` [2] **role=standalone** — sense=rebuke; type=action; operation=rebuke; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **anger** `H7110` [1/M02] **role=process-qualifier** — sense=anger; type=status; operation=(qualifies) rebuke; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed); manner=manner-of rebuke; coupling=welds rebuke
- **discipline** `H3256` [2] **role=standalone** — sense=discipline; type=action; operation=discipline; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **wrath** `H2534` [1/M02] **role=process-qualifier** — sense=wrath; type=status; operation=(qualifies) discipline; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed); manner=manner-of discipline; coupling=welds discipline

## Psa 38:2

- **arrows** `H2671` [2] **role=standalone** — sense=arrows; type=status
- **sunk** `H5181` [2] **role=standalone** — sense=sunk; type=action; operation=sunk
- **come down** `H5181` [2] **role=standalone** — sense=come down; type=action; operation=come down

## Psa 38:3

- **no** `H0369` [2] **role=process-qualifier** — sense=no; type=status; seat=flesh
- **soundness** `H4974` [1/M12] **role=characteristic** — sense=soundness; type=status
- **flesh** `H1320` [1/M47] **role=characteristic** — sense=flesh; type=status; seat=flesh
- **because** `H6440` [2] **role=standalone** — sense=because; type=status
- **indignation** `H2195` [1/M02] **role=characteristic** — sense=indignation; type=status
- **no** `H0369` [2] **role=standalone** — sense=no; type=status
- **health** `H7965` [1/M33] **role=characteristic** — sense=health; type=status
- **because** `H6440` [2] **role=standalone** — sense=because; type=status
- **sin** `H2403` [1/M10] **role=characteristic** — sense=sin; type=status

## Psa 38:4

- **iniquities** `H5771` [1/M10] **role=characteristic** — sense=iniquities; type=status
- **gone** `H5674` [2] **role=standalone** — sense=gone; type=action; operation=gone
- **head** `H7218` [2] **role=standalone** — sense=head; type=status
- **heavy** `H3515` [2] **role=standalone** — sense=heavy; type=quality
- **burden** `H4853` [2] **role=process-qualifier** — sense=burden; type=status; operation=(qualifies) heavy; manner=manner-of heavy; coupling=welds heavy
- **heavy** `H3513` [1/M22] **role=characteristic** — sense=heavy; type=action; operation=heavy

## Psa 38:5

- **wounds** `H2250` [2] **role=standalone** — sense=wounds; type=status
- **stink** `H0887` [1/M06] **role=characteristic** — sense=stink; type=action; operation=stink
- **fester** `H4743` [2] **role=standalone** — sense=fester; type=action; operation=fester
- **because** `H6440` [2] **role=process-qualifier** — sense=because; type=status; operation=(qualifies) fester; manner=manner-of fester; coupling=welds fester
- **foolishness** `H0200` [1/M16] **role=characteristic** — sense=foolishness; type=status

## Psa 38:6

- **bowed** `H5753` [1/M10] **role=characteristic** — sense=bowed; type=action; operation=bowed; intensity=all, very
- **down** `H7817` [2] **role=standalone** — sense=down; type=action; operation=down; intensity=all, very
- **all** `H3605` [2] **role=standalone** — sense=all; type=status; intensity=all, very
- **day** `H3117` [2] **role=standalone** — sense=day; type=status; intensity=all
- **go** `H1980` [2] **role=standalone** — sense=go; type=action; operation=go; intensity=all

## Psa 38:7

- **sides** `H3689` [2] **role=standalone** — sense=sides; type=status
- **filled** `H4390` [2] **role=standalone** — sense=filled; type=action; operation=filled
- **burning** `H7033` [2] **role=standalone** — sense=burning; type=action; operation=burning
- **no** `H0369` [2] **role=standalone** — sense=no; type=status
- **soundness** `H4974` [1/M12] **role=characteristic** — sense=soundness; type=status
- **flesh** `H1320` [1/M47] **role=characteristic** — sense=flesh; type=status; seat=flesh

## Psa 38:8

- **feeble** `H6313` [2] **role=standalone** — sense=feeble; type=action; operation=feeble; intensity=very
- **groan** `H7580` [1/M42] **role=characteristic** — sense=groan; type=action; operation=groan; intensity=very
- **tumult** `H5100` [1] **role=characteristic** — sense=tumult; type=status; seat=heart; intensity=very
- **heart** `H3820` [1/M47] **role=characteristic** — sense=heart; type=status; seat=heart

## Psa 38:9

- **Lord** `H0136` [2] **role=standalone** — sense=Lord; type=status; intensity=all
- **all** `H3605` [2] **role=standalone** — sense=all; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed); intensity=all
- **longing** `H8378` [1/M28] **role=characteristic** — sense=longing; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed); intensity=all
- **before** `H5048` [2] **role=standalone** — sense=before; type=quality; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed); intensity=all; prohibition=forbidden (neg particle)
- **sighing** `H0585` [1/M03] **role=characteristic** — sense=sighing; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed); prohibition=forbidden (neg particle)
- **hidden** `H5641` [2] **role=standalone** — sense=hidden; type=action; operation=hidden; prohibition=forbidden (neg particle)

## Psa 38:10

- **heart** `H3820` [1/M47] **role=characteristic** — sense=heart; type=status; seat=heart
- **throbs** `H5503` [2] **role=standalone** — sense=throbs; type=action; operation=throbs
- **strength** `H3581` [1/M23] **role=characteristic** — sense=strength; type=status
- **fails** `H5800` [2] **role=standalone** — sense=fails; type=action; operation=fails
- **light** `H0216` [2] **role=standalone** — sense=light; type=status
- **eyes** `H5869` [2] **role=standalone** — sense=eyes; type=status
- **gone** `H0369` [2] **role=standalone** — sense=gone; type=status

## Psa 38:11

- **friends** `H0157` [1/M05] **role=characteristic** — sense=friends; type=action; operation=friends
- **stand** `H5975` [2] **role=standalone** — sense=stand; type=action; operation=stand
- **aloof** `H5048` [2] **role=standalone** — sense=aloof; type=quality
- **plague** `H5061` [2] **role=standalone** — sense=plague; type=status
- **nearest kin** `H7138` [2] **role=standalone** — sense=nearest kin; type=quality
- **stand** `H5975` [2] **role=standalone** — sense=stand; type=action; operation=stand
- **far** `H7350` [2] **role=standalone** — sense=far; type=quality

## Psa 38:12

- **seek** `H1245` [1/M41] **role=characteristic** — sense=seek; type=action; operation=seek
- **life** `H5315` [1/M25] **role=characteristic** — sense=life; type=status; seat=soul
- **lay their snares** `H5367` [2] **role=standalone** — sense=lay their snares; type=action; operation=lay their snares
- **seek** `H1875` [1/M41] **role=characteristic** — sense=seek; type=action; operation=seek
- **hurt** `H7451` [1/M03] **role=characteristic** — sense=hurt; type=status
- **speak** `H1696` [2] **role=standalone** — sense=speak; type=action; operation=speak
- **ruin** `H1942` [1/M28] **role=characteristic** — sense=ruin; type=status; intensity=all
- **meditate** `H1897` [1/M42] **role=characteristic** — sense=meditate; type=action; operation=meditate; intensity=all
- **treachery** `H4820` [1/M14] **role=characteristic** — sense=treachery; type=status; intensity=all
- **all** `H3605` [2] **role=standalone** — sense=all; type=status; intensity=all
- **day** `H3117` [2] **role=standalone** — sense=day; type=status; intensity=all

## Psa 38:13

- **deaf** `H2795` [2] **role=standalone** — sense=deaf; type=quality; prohibition=forbidden (neg particle)
- **hear** `H8085` [1/M41] **role=characteristic** — sense=hear; type=action; operation=hear; prohibition=forbidden (neg particle)
- **mute man** `H0483` [2] **role=standalone** — sense=mute man; type=quality; prohibition=forbidden (neg particle)
- **open** `H6605` [2] **role=standalone** — sense=open; type=action; operation=open; prohibition=forbidden (neg particle)
- **mouth** `H6310` [2] **role=standalone** — sense=mouth; type=status; prohibition=forbidden (neg particle)

## Psa 38:14

- **become** `H1961` [2] **role=standalone** — sense=become; type=action; operation=become; prohibition=forbidden (neg particle)
- **man** `H0376` [2] **role=process-qualifier** — sense=man; type=status; operation=(qualifies) become; manner=manner-of become; coupling=welds become; prohibition=forbidden (neg particle)
- **hear** `H8085` [1/M41] **role=characteristic** — sense=hear; type=action; operation=hear; prohibition=forbidden (neg particle)
- **mouth** `H6310` [2] **role=process-qualifier** — sense=mouth; type=status; operation=(qualifies) hear; manner=manner-of hear; coupling=welds hear; prohibition=forbidden (neg particle)
- **rebukes** `H8433` [2] **role=standalone** — sense=rebukes; type=status; prohibition=forbidden (neg particle)

## Psa 38:15

- **Lord** `H3068` [2] **role=process-qualifier** — sense=Lord; type=status; operation=(qualifies) wait; manner=manner-of wait; coupling=welds wait
- **wait** `H3176` [1/M18] **role=characteristic** — sense=wait; type=action; operation=wait; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **Lord** `H0136` [2] **role=standalone** — sense=Lord; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **God** `H0430` [2] **role=standalone** — sense=God; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **answer** `H6030` [2] **role=standalone** — sense=answer; type=action; operation=answer; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)

## Psa 38:16

- **said** `H0559` [2] **role=standalone** — sense=said; type=action; operation=said
- **rejoice** `H8055` [1/M04] **role=characteristic** — sense=rejoice; type=action; operation=rejoice
- **boast** `H1431` [1/M22] **role=characteristic** — sense=boast; type=action; operation=boast
- **slips** `H4131` [2] **role=process-qualifier** — sense=slips; type=status; operation=(qualifies) boast; manner=manner-of boast; coupling=welds boast

## Psa 38:17

- **ready** `H3559` [2] **role=standalone** — sense=ready; type=action; operation=ready
- **fall** `H6761` [2] **role=process-qualifier** — sense=fall; type=status; operation=(qualifies) ready; manner=manner-of ready; coupling=welds ready
- **pain** `H4341` [1/M03] **role=characteristic** — sense=pain; type=status
- **ever** `H8548` [2] **role=standalone** — sense=ever; type=status
- **before** `H5048` [2] **role=standalone** — sense=before; type=quality

## Psa 38:18

- **confess** `H5046` [2] **role=standalone** — sense=confess; type=action; operation=confess
- **iniquity** `H5771` [1/M10] **role=characteristic** — sense=iniquity; type=status
- **sorry** `H1672` [1/M20] **role=characteristic** — sense=sorry; type=action; operation=sorry
- **sin** `H2403` [1/M10] **role=process-qualifier** — sense=sin; type=status; operation=(qualifies) sorry; manner=manner-of sorry; coupling=welds sorry

## Psa 38:19

- **foes** `H0341` [1/M44] **role=standalone** — sense=foes; type=action; operation=foes
- **vigorous** `H2416` [1/M44] **role=characteristic** — sense=vigorous; type=quality
- **many** `H7231` [2] **role=standalone** — sense=many; type=action; operation=many
- **hate** `H8130` [1/M06] **role=characteristic** — sense=hate; type=action; operation=hate
- **wrongfully** `H8267` [1/M14] **role=characteristic** — sense=wrongfully; type=status

## Psa 38:20

- **render** `H7999` [2] **role=standalone** — sense=render; type=action; operation=render
- **evil** `H7451` [1/M27] **role=characteristic** — sense=evil; type=status
- **for** `H8478` [2] **role=standalone** — sense=for; type=status
- **good** `H2896` [1/M04] **role=characteristic** — sense=good; type=status
- **accuse** `H7853` [2] **role=standalone** — sense=accuse; type=action; operation=accuse
- **because** `H8478` [2] **role=standalone** — sense=because; type=status
- **follow** `H7291` [2] **role=standalone** — sense=follow; type=action; operation=follow
- **good** `H2896` [1/M04] **role=characteristic** — sense=good; type=quality

## Psa 38:21

- **forsake** `H5800` [2] **role=standalone** — sense=forsake; type=action; operation=forsake
- **Lord** `H3068` [2] **role=standalone** — sense=Lord; type=status
- **God** `H0430` [2] **role=standalone** — sense=God; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)
- **far** `H7368` [2] **role=standalone** — sense=far; type=action; operation=far; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)

## Psa 38:22

- **Make haste** `H2363` [2] **role=standalone** — sense=Make haste; type=action; operation=Make haste
- **help** `H5833` [2] **role=process-qualifier** — sense=help; type=status; operation=(qualifies) Make haste; manner=manner-of Make haste; coupling=welds Make haste
- **Lord** `H0136` [2] **role=standalone** — sense=Lord; type=status
- **salvation** `H8668` [2] **role=standalone** — sense=salvation; type=status; bearer=Lord; discovery=bearer unreliable (nearest-proper heuristic; subject-agreement not parsed)