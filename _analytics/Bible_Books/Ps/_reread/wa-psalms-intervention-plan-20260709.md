...
- **Phase 1 — char-driven lexical completion (cycle §5 reading discipline).** **The characteristic is the lens: work one char at a time.** For each char, **resolve its pairs by reading across the passage using the morphology of the related verses** — its target(+object-type), bearer, seat, manner, coupling — grounded, not auto-paired from a span-sweep. Take the mechanical dims from morphology; **record the operation**; write **explicit `none`** for any mandatory dim the verse is silent on (P4); encode every pair endpoint as a **span-id**. Run the **discovery-lookout** per verse (a finding or `discovery: none`). *(Genre-aware: source/effect are Phase-2 chapter-level for poetic — §4.1.)*
