...
This is the **same three layers**: the Logos capture is a *source* that feeds *atoms* (observations) and the *story* (prose). It dovetails with D2 (revive `prose_section`) — the capture is just one more section type.
