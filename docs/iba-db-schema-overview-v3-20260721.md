...
- Generated (UTC): 2026-07-21T05:18:33Z
- Tables: 33
- Views: 1
- Triggers: 0
- Indexes: 28
...
- vw_passages_by_book: CREATE VIEW vw_passages_by_book AS SELECT     p.*,     v.reference AS verse_reference,     v.text      AS verse_text,     (         SELECT GROUP_CONCAT(cc.candidate_tag, ', ')         FROM (             SELECT DISTINCT sc.candidate_tag             FROM span s             JOIN span_candidate sc ON sc.span_id = s.id AND sc.deleted = 0             WHERE s.verse_id = v.id AND s.deleted = 0             ORDER BY sc.candidate_tag         ) cc     ) AS candidate_chars FROM passage p JOIN verse_passage vp ON vp.passage_id = p.id AND vp.deleted = 0 JOIN verse v          ON v.id = vp.verse_id AND v.deleted = 0 WHERE p.deleted = 0 ORDER BY p.book, p.start_chapter, p.start_verse, v.id
