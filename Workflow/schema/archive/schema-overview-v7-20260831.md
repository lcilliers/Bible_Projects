...
> Generated 2026-08-31T13:13:43Z by `report.schema_overview`. Introspects the live DB directly and merges it against `cfg_table`/`cfg_column` (database='iba') — always current, never hand-maintained.
...
| cfg_api | — | the catalogue of STEP Bible REST API calls the app actually issues — route templ | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_behaviour_class | — | the taxonomy for governance.operational_behaviour_control -- chat, terminal, sql | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_behaviour_rule | — | the actual rule content per class -- worded as definitive statements, not open f | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_book_order | — | canonical book ordering (Gen=0 .. Rev=65) for sorting/sequencing verse reference | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_candidate_rule | — | accept/reject overrides for candidate Strong's numbers considered during term/HI | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_change_detail | — | row-level audit of every cfg_* change actually applied — what cfg_change_log's w | log | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_change_log | — | audit trail of cfg_* seed reloads — config_version and seed_hash (change-detecti | log | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_column | — | the column-level half of governance.table_columns — every column's type, key/nul | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_connection | — | STEP Bible local-server connection parameters (base_url, version, ...) — read at | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_content_index_exclude | — | governs content_index.rebuild/.refresh's file scope: any .md file whose path sta | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_content_index_size_override | — | overrides content_index.exclude_size_threshold_bytes: a .md file matching an ACT | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_enum | — | named controlled-vocabulary groups — lookups/options queried BY NAME at runtime  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_escalation | — | One row per discrete, nameable rule governing the escalation utility itself -- c | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_escalation_requirement | — | Which fields are required for which escalation action (comment@raise, resolution | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_escalation_transition | — | The escalation state-derivation rule engine, evaluated in priority order per sha | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_index | — | Secondary (non-unique) indexes to build per data table -- closes the gap left by | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_meta | — | core app-identity facts (which database this cfg_* store belongs to, the seeded  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_method_rule | — | One row per discrete, nameable analytical rule governing a debate-pipeline step  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_on_fail | — | how a named step should react to a named failure condition — path (the actual ro | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_passage | — | Module-specific settings for the passage module (governance.module.config) -- re | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_prose | — | Module-specific settings for the prose module (governance.module.config) -- chap | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_prose_concept | — | a pointer index: 'this concept is DEFINED at this chapter/section of the prose'  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_quality_check | — | Config-defined reasonability/existence self-check questions per debate-pipeline  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_report | — | report-generation shape for a step per governance.reports_must_persist — title,  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_report_csv_table | — | which table(s) a report step's CSV output covers — join_note describes a multi-t | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_report_section | — | the section layout of a generated report — ordinal position, the markdown headin | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_setting | — | flat key/value application settings, grouped by module (cfg_setting.module) — th | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_status_flow | — | the ordered status lifecycle for a named entity (e.g. 'word') — which step sets  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_step | — | the dispatcher's step registry — which handler function runs for a named step wi | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_table | — | the table-level half of governance.tables — every table across both project data | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_table_purpose | — | Per genuine cfg_* RULE table (never a data table, never a demoted log table like | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_unique | — | documents a compound (multi-column) uniqueness expectation for a table — table_n | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_utility | — | the registry of every script/routine in the project per governance.scripts_and_r | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_work_package | — | the dispatcher's top-level package registry — its PowerShell entry script, what  | rule | **NO LIVE TABLE** (stale cfg_table row) |
| cfg_write_grant | — | governance.config_control's write-grant registry — which writer (a step name, or | rule | **NO LIVE TABLE** (stale cfg_table row) |
...
| run | 2359 | what ran, pinned to a config version, and RESUMABLE (O7): state + resume_point p | data |  |
...
- 41 live table(s), 76 registered in `cfg_table` (database='iba'), **36 discrepancies**.
