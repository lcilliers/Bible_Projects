...
> Generated 2026-08-31T13:16:02Z by `report.schema_overview`. Introspects the live DB directly and merges it against `cfg_table`/`cfg_column` (database='iba') — always current, never hand-maintained.
...
| escalation_history | 2318 | One row per update to an item, ever -- append-only, a TRUE DELTA per version (mo | data |  |
...
| run | 2362 | what ran, pinned to a config version, and RESUMABLE (O7): state + resume_point p | data |  |
