...
> **⚠ CLOSED 2026-07-08 — content morphed into the current model; retained for provenance only.** Its **item framing** was absorbed into the verse-first **VE-lexical catalogue** (`Workflow/Catalogue/wa-ve-lexical-catalogue-v1-20260702.md`, the current authority; the reset's §3 considerations are the catalogue's "converges legacy" column). Its **governing principles** (P0–P8, functional-first, the mandatory discovery-lookout) were folded into the authoritative cycle instruction **§3A** (`wa-characteristic-role-lexical-cycle-authoritative-v1-20260708.md`). Its **assembly half** (`wa-synthesis-B-spec-reset-v1`) remains relevant (stale). Do not use this doc for new work — cite the catalogue + cycle §3A instead.
...
- **A flag that signals the verse needs interpretive resolution beyond the mechanical** — a **figurative/somatic-metaphor** ("clean hands" = conduct), a **distributed movement** (effect in the next verses), **heavy UNRESOLVED**, or significant lookup-flagged content — **routes the verse to the L1.5 full-exegesis (Logos) GATE** (`wa-synthesis-B-spec-reset-v1 §2b`): a researcher-led, AI-assisted exegesis done **after** this lexical analysis and **before** synthesis, whose result lands in the DB (read/exegesis-provenance, preserved). The mechanical pass is breadth; the flagged minority gets depth-on-demand.
