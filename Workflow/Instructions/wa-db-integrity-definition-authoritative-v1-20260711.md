...
| **I2 Master-index coverage** (model **(b)**, researcher 2026-07-11) | `wa_verse_records` is **one row per term-in-verse** (not per occurrence). A char-span is **covered** when its **(verse, base-Strong's term)** has ≥1 active `wa_verse_records`. When a term is a characteristic **more than once** in a verse, the single record covers all its occurrences. | CLAUDE.md §3; researcher decision (b) | char-spans whose (verse,term) has **no** active record = 0 |
| **I2b Link quality** (tracking, not a coverage violation) | Each active `wa_verse_records` should carry a valid `verse_span_id` to one occurrence-span (so records trace to the master). A null/stale `verse_span_id` on a covered (verse,term) is a **cleanup item**, not an I2 violation. | resolve at read-time (db.resolve) | active records with `verse_span_id IS NULL` for a read verse → link |
