# Verse-analysis method — v1.1 (the live lexical process, 2026-07-02; §15 added 2026-07-03)
...
>
> **Changelog:** v1 (2026-07-02) — verse-first/passage/term-driven core + §14 poetic chapter-driven method. **v1.1 (2026-07-03)** — Phase-1 lexical enhanced (construct-chain specifier ve_nr 110, positional-object inference, target role-fix; see memory `project_poetic_chapter_driven_method`); **§15 added: the SEGMENTATION-FIRST variant** for discourse-shaped books (Proverbs, Ecclesiastes, Job, Lamentations).
...
**Learnings captured this run (Psalms 1–4 — continuous-improvement loop):**
- **Stop-list rule (learned Psa 2–4, in the script):** a gate-1 tagged term in `STOPLIST_NOT_CHARACTERISTIC` is never an inner-being characteristic → standalone. Two classes: **superscription metadata** (*mizmor* H4210 "Psalm", *shir* H7892 "Song", *menatseach* H5329 "choirmaster") and **external-entity adversaries** (*enemy* H0341, *foe/adversary* H6862). Lemma-level, not cluster-level — M44 (Relational) is mixed (holds *covenant/companion/betroth* as well as *enemy*), so a cluster rule would over-demote. Extend the list as verified lemmas appear.
...
## 15. DISCOURSE-SHAPED BOOKS — segmentation-first variant (2026-07-03)

For the specialised books that are **argument/dialogue/discourse** rather than lyric (Proverbs, Ecclesiastes, Job, Lamentations), the chapter (§14) is too coarse and the term-driven passage (§1–3) does not apply — the natural unit is the **inner-being segmentation unit**. The pipeline extends §14 with a segmentation step between Phase 1 and Phase 2. **Governing question is unchanged: "what do these verses tell us about the inner being?"**

**Pipeline: Phase 0 backfill → Phase 1 verse lexical → SEGMENTATION → Phase 2 per-unit meaning-synthesis.**

- **Phase 0 — backfill** (as §14): `scripts/_apply_backfill_chapter_verses_v1_20260702.py --book --chapter --live` to bring every chapter to its full canonical verse count (STEP-probed). Genre inherited (Job = `poetic/wisdom`, Lam = `prophetic`; both processed in poetic mode — cross-verse OFF).
- **Phase 1 — verse lexical** (as §14, v1.1 substrate): `scripts/_apply_poetic_chapter_lexical_v1_20260702.py --book --chapter --live [--no-backup]`. `--no-backup` for mass re-runs (DB is backed up daily to NAS).
- **SEGMENTATION (new).** Read the whole book and divide it into **inner-being units** — a unit is a run of verses that carries one inner-being movement (or several held together). **Book-agnostic** store: `segment_unit` + `segment_unit_verse` (M:N). Author a JSON (`verse-analysis/<book>/_seg/<book>-segmentation-chA-B.json`) and load with `scripts/_apply_load_segmentation_v1_20260703.py --in PATH [--live]` (idempotent **per chapter**; dry-run reports units/verse-links/unresolved). Fields per unit: `code`, `chapter` (one per unit), `type`, `characteristics` (provisional — validated/revised in Phase 2), `multi` (true where several characteristics operate together — researcher's explicit requirement), `is_thread`, `gist`, `verses` (ints and "a-b" ranges within the chapter). **Unit types:** `D` discourse · `S` single-saying · `C` cluster · `T` thread (recurring, e.g. Ecc's joy-refrain) · `F` frame/arena (e.g. Ecc 1:2-11 the vanity frame). Segmentation is verse-bounded and provisional; the characteristics named are **candidates the Phase-2 reading confirms, revises, or replaces** — never imputed (see [[feedback_characteristic_list_validates_not_imputes]]).
- **Phase 2 — per-unit meaning-synthesis.** For each unit: (a) **the reading** — the inner-being operation/finding, verse-bounded, inferences tagged *stated* | *inferred*, lens on the operation not the God-relation; (b) **lexical validation (read-back)** — lay the unit's ve_lexical beside the text (`scripts/_inspect_unit_lexical_v1_20260703.py --book --chapter N`), check roles/pairs/seats/flags for sensibility, log systemic residuals (do not re-derive); (c) **hidden/less-obvious characteristics isolated** — what the provisional segmentation missed; (d) **characteristics surfaced** (new vs shared-with-other-books). Plus chapter-level notes (validation summary, new characteristics, threads). File one prose_section per chapter via `scripts/_apply_file_chapter_lexical_prose_v1_20260702.py --book --chapter --story --live` (type `lexical_prose_chapter`, version-aware).

**Systemic Phase-1 residuals to log (not defects in the reading — candidates for rule-adjustment):** (1) `seat=spirit` mis-fires on **elemental ruach** ("striving after wind", wind idiom) — correct only where the surface is human breath/spirit (validated across Ecc 3:19/12:7, Job 34:14); (2) the **positional-target** rule fires on poetic adjacent nouns (self-flagged `unverified`) — over-produces in verb-heavy meditation; (3) **neg-particle `prohibition`** over-spreads from the negated verb to neighbouring tokens; (4) **bearer=nearest-proper bleed** (e.g. bearer=LORD stamped on a clause where a human is the actor). The readings are read from the verse structure, not the tags, where these fire.

**Executed under this variant (2026-07-03):** Proverbs (31 ch, 150 units), Ecclesiastes (12 ch, 47 units, prose_section 624-635), Job (42 ch, 101 units, 636-677), Lamentations (5 ch, 16 units, 678-682). All DB-canonical + reproducible. **Wisdom/poetry corpus (Ps+Pro+Ecc+Job+Lam) COMPLETE.** Coverage baseline for next books: `verse-analysis/wa-book-coverage-assessment-20260703.md`. Governing guardrail for the emerging characteristic tally: [[feedback_characteristic_list_validates_not_imputes]] (validation, never imputation).

### §15.1 — PROPHETIC books: oracle-passage sub-variant (2026-07-04)
Prophetic books are **not** processed by chapter and **not** as a whole-book blob — the unit is the **oracle** (a rhetorically-marked passage that routinely crosses chapter lines). First executed on **Malachi** (book 39, 55 verses): its form is the **disputation** (God's claim → the people's objection "But you say…" → God's answer), giving **6 disputations + intro + epilogue**; three of the six cross a chapter boundary (1:6–2:9, 2:17–3:5, 3:13–4:3). The disputation form is intrinsically inner-being — the objection-seam *stages the people's interior* (doubt, contempt, faithlessness, cynicism, withholding, the fearing remnant), which is the reading's spine.

**Reusable capability added:** the segmentation loader (`_apply_load_segmentation_v1_20260703.py`) now accepts a **`verse_refs`** field on a unit — a list of `"ch:vn"` / `"ch:a-b"` entries — so a single unit can span chapters (an oracle). `chapter`/`verses` (single-chapter) still work; `verse_refs` takes precedence and sets the anchor chapter to the first ref. Cross-chapter oracle readings are filed under their **anchor chapter** (each oracle read over its full range). Scout a prophet's oracle structure first (as in `wa-malachi-structure-analysis-20260704.md`) before segmenting. Malachi readings: prose_section 703–706.
