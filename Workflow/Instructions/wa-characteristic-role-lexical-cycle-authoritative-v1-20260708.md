...
- **Source:** built **from the master + the lexical + the verse-record morphology** (no new reading) — `verse_span_index` char-spans × `ve_lexical` 101/106 × `wa_verse_records` stem/morph/target_word × `mti_terms` gloss. Legacy pre-read rows and the superseded v2 records are archived to `ib_characteristic_legacy` / exported to `outputs/data/ib_characteristic_v2_*` before rebuild (reversible).
