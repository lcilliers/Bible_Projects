...
2. **Read char-driven** (cycle §5): for each candidate char — Screen 0 (IB-relevance; God = arena not subject) → role (characteristic / qualifier / standalone / undecided) → decompose across the **M16 mandatory ledger** (101,102,104,105,106,107,108,**109,110,111**,112,114,115,116,**117,118**), resolving pairs by reading across the passage; **`none` written explicitly, never omitted**. **★ Qualifier/device step (2026-07-14):** *type each surrounding qualifier's relationship to the char* — a **modifying** qualifier → `intensity`(109)/`specifier`(110)/`effect`(111) **and linked as a typed pair** (never left `standalone`); the **device** through which the char is expressed → `device`(117), with the analogy/simile/metaphor **vehicle as the pair endpoint**; the movement vector → `direction`(118). **Every dimension value must be self-interpretable — readable without the verse** (carry the trigger + the sense; per the catalogue's self-interpretable rule).
