...
- **v1.2's ABSENT dims — precise diagnosis (researcher query, 2026-07-14; checked against the whole `ve_lexical` table, not just the reread).** All three are ABSENT *in the poetic reread*, but they are **not** the same case, and only one is truly redundant:
  - **`intensity`(109)** — **8,748 rows in the OLD `lexical-model-2026` layer**, almost all the bare adverb `"very"`. The reread folded degree into the prose. *Design-dropped, not redundant* — a genuinely useful facet (how strong is the fear/joy/anger?) that was thinly implemented; an **under-served gap** worth authoring properly for future books, if degree is analytically wanted.
  - **`specifier`(110)** — **9,794 rows in the OLD layer**: `"of David" / "of Solomon" / "of Israel"` — **bibliographic/attributive, not inner-being.** **The one genuine redundancy** — a deprecation candidate on a par with the dropped D10/D12/D13.
  - **`effect`(111)** — 168 rows, mostly **Leviticus** + flagged **Phase-2 (cross-verse)** for poetry. **Not redundant** — it has a defined home (chapter/book level), just not per-span in the poetic method.
  - `direction`, `source`(partial) — direction never read (gap, authored forward); source is Phase-2/partial.
- **Projection decision:** keep 109/110/111/direction as **ABSENT columns** — the ABSENT signal is *itself information* ("this facet was not measured here"), and dropping them would hide that a facet was never captured. But the preamble states the three distinct reasons (dropped-thin / deprecate-bibliographic / Phase-2), and **`specifier` may be dropped from the poetic projection** on the researcher's call.
