...
### Finding 4 — 27 of 99 active catalogue questions are structurally unreachable by ANY stage, not just "not yet answered"
...
full catalogue. Doing that now, by `scope` (99 active questions, live):

| scope | total | answered | mechanism |
|---|---:|---:|---|
| Characteristic (HIB behaviour) | 24 | 20 | char-answers, scope-driven query — all 4 zero are the documented science-extract exclusion (§4.2) |
| Characteristic relational | 17 | 17 | char-answers — full coverage |
| Other non-human beings | 12 | 12 | char-answers — full coverage |
| The HIB | 4 | 4 | char-answers — full coverage |
| **The verse** | 5 | 1 | only `M0.6.5` is hardcoded into Stage 1's query; the other 4 (`M0.6.1`–`M0.6.4`) share this exact scope label but no stage ever asks them |
| **Verse-context** | 10 | 0 | no stage's query anywhere selects `scope='Verse-context'` — the entire scope is untouched, by construction |
| **Word/term (lexical)** | 27 | 14 | Stage 1 selects by a HARDCODED literal list (`question_code LIKE 'M0.1%' OR LIKE 'M0.5%' OR IN ('D7.7.1','M0.6.5')`), not by scope — 14 of 27 happen to be `M0.1.*`/`M0.5.*` and get answered; the other 13 share the scope label but were never added to that hardcoded list |

**Root cause, precisely:** Stage 4 (`char-answers`) selects its battery by `scope IN (...)` —
a live, dynamic query (§4.2) that automatically covers a newly-added question under one of its 4
scopes. Stage 1 (`verse-reading`) selects by a **hardcoded set of literal question codes**, not by
scope — so any question whose `scope` happens to be `'The verse'`/`'Verse-context'`/`'Word/term
(lexical)'` but whose code isn't literally `M0.1.x`/`M0.5.x`/`D7.7.1`/`M0.6.5` is invisible to
every stage's SQL, permanently, regardless of how many clusters get processed. This is not "hasn't
gotten to them yet" — running more clusters through the existing pipeline will never answer these
27 questions, because no code path ever asks for them.

27 of the 31 zero-observation questions found in this conversation's earlier exploration are this
gap; the other 4 (`D9.1.1`/`D9.2.1`/`D11.2.1`/`D12.1.1`) are the separately-documented,
already-known science-extract exclusion.
