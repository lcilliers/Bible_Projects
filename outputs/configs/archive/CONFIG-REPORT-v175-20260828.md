...
| generated_at | 2026-08-28T09:36:32Z |
...
1. cfg_utility 'pathaudit' (iba\app\lib\pathaudit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
...
2. cfg_prose.prose.edit_file_dir = '"outputs/markdown/prose-edits"' — 'outputs/markdown/prose-edits' does not exist as a folder on disk (project-root-relative)
...
**413** registered module(s) — **31** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **360** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
| pathaudit | iba/app/lib/pathaudit.py | pathaudit.py -- project-wide scan for hardcoded folder/file-path string literals not backed by a live cfg accessor. Escalation #971/#976, the automated successor to the one-off #648 sweep for the location subset specifically. | ✓ |  |  |
