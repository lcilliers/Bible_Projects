...
| generated_at | 2026-08-06T09:18:55Z |
...
1. cfg_setting 'passage.default_rule' (key not found together with a cfg.setting(...) call in any one file)
2. cfg_setting 'passage.cross_chapter' (key not found together with a cfg.setting(...) call in any one file)
3. cfg_setting 'passage.min_shared_hibs' (key not found together with a cfg.setting(...) call in any one file)
...
4. passage.book_label filled_by='report.verse_span_meaning' (an inactive step) — confirm dormant or update to the real current writer
5. passage.verse_span_meaning_path filled_by='report.verse_span_meaning' (an inactive step) — confirm dormant or update to the real current writer
6. passage.verse_span_meaning_written_at filled_by='report.verse_span_meaning' (an inactive step) — confirm dormant or update to the real current writer
7. passage.debate_path filled_by='report.passage_debate' (an inactive step) — confirm dormant or update to the real current writer
8. passage.debate_written_at filled_by='report.passage_debate' (an inactive step) — confirm dormant or update to the real current writer
9. passage.debate_status filled_by='report.passage_debate' (an inactive step) — confirm dormant or update to the real current writer

**Stale governance docs** (1) — GOVERNANCE.md older than the newest applied config change:
10. GOVERNANCE.md was last modified 2026-08-02T13:46:47Z, before the newest applied cfg_change_detail row (2026-08-05T20:05:41Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)
...
- **cfg_on_fail** (12): `candidate.curate/change-rejected`, `candidate.curate/invalid-proposal`, `candidate.curate/needs-approval`, `candidate.curate/needs-revision`, `candidate.load/needs-review`, `candidate.seed/no-inventory`, `candidate.set/no-spans`, `candidate.validate/findings-rejected`, `candidate.validate/needs-review`, `candidate.validate/needs-revision`, `passage.build/no-candidates`, `report.verse_span_meaning/unreachable`
- **cfg_candidate_rule** (by kind): accept=289

## 2. Utilities registry

**25** registered module(s) — **12** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **0** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
**14 of 64 conditions ESCALATE** (pause-continue — the researcher is asked); the rest either stop the run outright (report-stop) or continue with a logged warning (report-continue). Per the researcher's 2026-07-21 rule: any finding that needs a judgement call must be in the first group, not silently in the second or third.
...
| passage.debate_sync | debate-file-missing | report-stop | the tracked debate_path no longer exists on disk — was it moved or deleted outside the app? |
| passage.debate_sync | no-debate-file | report-stop | no report.passage_debate output tracked yet for this exact book/range — run PassageDebate-Report.ps1 first (report.passage_debate) |
...
| hib.set | hib, hib_referent_option, verse_hib |
...
| operation.set | operation, operation_party |
| passage.build | passage, verse_passage |
| phenomenon.set | passage, phenomenon |
...
| passage_rule | hib-continuity, maximal |
...
work package `chapter-generate` → `iba/app/ps/Chapter-Generate.ps1` (chained=1)
- on completion: _chapter data + debate scaffold written -- fill in every <!-- fill in --> placeholder, then run PassageDebate-Sync.ps1 once done. Recommended: ~passage.debate_session_chapter_guideline chapters per Claude Code session, then clear context before continuing._
- next-step hint: _iba/app/ps/PassageDebate-Sync.ps1 -Book <book> -Chapters <n> -BookLabel <label>   # after manually filling the scaffold_

| # | section | heading | toc label | in ToC |
| --- | --- | --- | --- | --- |
| 0 | preliminaries | ## Preliminaries | Preliminaries | ✓ |
| 1 | phenomena_register | ## Phenomena register (Phase 1 output) | Phenomena register (Phase 1 output) | ✓ |
| 2 | operations | ## Per-verse operations (Phase 2 output) | Per-verse operations (Phase 2 output) | ✓ |
| 3 | linkages | ## Passage-level linkages (Q7) | Passage-level linkages (Q7) | ✓ |
| 4 | insufficiencies | ## Insufficiencies register | Insufficiencies register | ✓ |
| 5 | emergent | ## Emergent questions log (filed against this passage — not merged across passages; resolved, if at all, at the whole-book read) | Emergent questions log (filed against this passage — not merged across passages; resolved, if at all, at the whole-book read) | ✓ |
| 6 | validation | ## Debate quality validation (Phase 3 output) | Debate quality validation (Phase 3 output) | ✓ |
| 7 | open_decisions | ## Open decisions / next steps | Open decisions / next steps | ✓ |
