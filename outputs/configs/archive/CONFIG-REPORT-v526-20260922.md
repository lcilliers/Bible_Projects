...
| generated_at | 2026-09-22T18:31:36Z |
...
34. GOVERNANCE.md was last modified 2026-09-22T07:28:18Z, before the newest applied cfg_change_detail row (2026-09-22T18:31:35Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)
...
**474** registered module(s) — **33** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **391** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
| lexical | lexical.meaning_max_strongs_per_batch | 8 | Cap on cumulative distinct M-code strongs per lexical.meaning batch -- the real cost driver for M0.7's front-loaded per-strong question volume (escalation #1825/#1826/#1827, 2026-09-21). Replaced the old verse-count cap (lexical.meaning_max_verses_per_batch, now inactive) after a fixed verse count was confirmed live to never reliably bound output when strong-density varies 3.5x between chunks. Read by _chunk_verses_by_strong_density() (iba/app/handlers/lexical.py) -- found live 2026-09-22 (escalation #1824 governance audit) that this setting was never actually registered despite being load-bearing since #311; the code was silently running on this same fallback value the whole time. |
| lexical | lexical.meaning_max_verses_per_batch | 5 | Dedicated batch-size override for lexical.meaning only (#1723) -- front-loading (every M-code strong in a batch verse-count, not just the cluster's own members) multiplies expected output roughly by strong-density-per-batch. The shared passage.max_verses (20) truncated a real batch (55 strongs) even at lexical.llm_max_output_tokens=20000 -- fixed then by halving to 10. REDUCED AGAIN 2026-09-21, escalation #1825: adding the M0.7.1-16 verse-substantiation family (#1806) roughly doubled the per-strong question count (14 -> 30), reproducing the identical truncation failure at the previous 10-verse cap (confirmed live, 2 consecutive M67 runs, both truncated mid-response around 85-90K characters). Halved again to 5, proportional to the roughly-doubled per-strong output volume, restoring the same safety margin the original #1723 fix established. Other passage.max_verses consumers (lexical.enrich) are unaffected by this override. -- SUPERSEDED 2026-09-21 by lexical.meaning_max_strongs_per_batch (escalation #1825/#1826/#1827): fixed verse-count chunking couldn't reliably bound output when strong-density varies (confirmed live, 3.5x spread across M67's own chunks). No live code reads this key anymore (confirmed live via grep, escalation #1824 governance audit, 2026-09-22). Marked inactive, not removed -- auditable history. |
