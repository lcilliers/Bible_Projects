...
| generated_at | 2026-09-18T08:21:52Z |
...
38. GOVERNANCE.md was last modified 2026-09-18T05:24:38Z, before the newest applied cfg_change_detail row (2026-09-18T08:21:51Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)

**Unregistered lib modules** (0) — iba/app/lib/*.py with no cfg_utility row:
_(none)_

**Low config-density utilities** (15) — NON-EXEMPT cfg_utility module with zero real Cfg-method call sites of its own (see §2 Utilities registry for the full module list, including the 11 already declared config_exempt):
...
**PS/worksheet drift** (7) — a script's live param() names not matching its tab's flag headers in governance.ps_worksheet_path:
55. CharAnswer.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\charanswer.ps1' in some tab's first rows)
56. CharReading.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\charreading.ps1' in some tab's first rows)
57. ClusterSubgroup.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\clustersubgroup.ps1' in some tab's first rows)
58. Config-Maintenance.ps1: ps tools worksheet.xlsx tab 'Config-Maintenance' is missing flag column(s) ['Title'] — the script has these parameters now
59. Lexical-Readiness.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\lexical-readiness.ps1' in some tab's first rows)
60. VerseMeta.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\versemeta.ps1' in some tab's first rows)
61. VerseReading.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\versereading.ps1' in some tab's first rows)
...
89. [sqlite] #1 verify-before-acting — context_delivered
90. [sqlite] #11 readonly-by-default — context_delivered
91. [sqlite] #13 dont-assume-which-database — context_delivered
92. [sqlite] #47 wa-session-research-flags-retained-as-is — context_delivered
93. [sqlite] #54 prose-quality-flag-on-upstream-change — context_delivered
94. [sqlite] #55 prose-section-type-creation-requires-researcher-instruction — context_delivered
95. [sqlite] #61 inactive-tables-never-active-inputs — context_delivered
...
**458** registered module(s) — **33** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **379** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
| cluster.answer | ib_node, ib_observation |
| cluster.assign | cluster_strong |
| cluster.reading | ib_node, ib_observation |
| cluster.subgroup | cluster_subgroup, cluster_subgroup_strong, ib_node, ib_observation |
...
| lexical.meaning | ib_node, ib_observation |
