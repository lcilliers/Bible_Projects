...
| generated_at | 2026-08-29T12:36:22Z |
...
1. cfg_prose.prose.patch_output_dir = '"Sessions/Patches"' — 'sessions/patches' does not exist as a folder on disk (project-root-relative)
...
2. iba/app/lib/prosestore.py builds a -v{n} filename by hand — no filingkit.versioned_path()/reportkit.oneoff_path() call site in the same file

**PS/worksheet drift** (1) — a script's live param() names not matching its tab's flag headers in governance.ps_worksheet_path:
3. Catalogue-Report.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\catalogue-report.ps1' in some tab's first rows)
...
4. Escalation.ps1 has parameter(s) ['ShortDescription', 'Source', 'Tried'] not used as a -Flag header anywhere in escalation actions worksheet.xlsx — the researcher's model sheet may need updating
...
| report | report.obs_catalogue_path | iba/app/reports/obs-catalogue.md | where report.obs_catalogue persists its output |
...
**Observation question catalogue — structural review** — output `md+csv` · naming `stable` · archived to `archive/` · ToC on
...
CSV pairing: `wa_obs_question_catalogue` (verbatim dump, no join -- but it's a bible_research.db table supplied via row_filter (cataloguereport.py connects cross-database itself), not a literal iba.db table write_csv_pairing could SELECT * from directly -- virtual=1 per find_bad_report_csv_table_references' documented row_filter exemption)
