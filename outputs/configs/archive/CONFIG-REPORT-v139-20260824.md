...
| generated_at | 2026-08-24T17:00:07Z |
...
**Orphan configs** (7) — a `cfg_setting`/`cfg_enum` not referenced by any code:
1. cfg_enum group 'prose_section_author' (not looked up by name at runtime — no cfg.enum('prose_section_author') or cfg_enum WHERE name='prose_section_author' call site)
2. cfg_enum group 'prose_section_status' (not looked up by name at runtime — no cfg.enum('prose_section_status') or cfg_enum WHERE name='prose_section_status' call site)
3. cfg_enum group 'prose_section_type_book_label' (not looked up by name at runtime — no cfg.enum('prose_section_type_book_label') or cfg_enum WHERE name='prose_section_type_book_label' call site)
4. cfg_enum group 'prose_section_type_lifecycle_tag' (not looked up by name at runtime — no cfg.enum('prose_section_type_lifecycle_tag') or cfg_enum WHERE name='prose_section_type_lifecycle_tag' call site)
5. cfg_enum group 'prose_section_type_source_stage' (not looked up by name at runtime — no cfg.enum('prose_section_type_source_stage') or cfg_enum WHERE name='prose_section_type_source_stage' call site)
6. cfg_enum group 'record_change_log_change_type' (not looked up by name at runtime — no cfg.enum('record_change_log_change_type') or cfg_enum WHERE name='record_change_log_change_type' call site)
7. cfg_enum group 'record_change_log_status' (not looked up by name at runtime — no cfg.enum('record_change_log_status') or cfg_enum WHERE name='record_change_log_status' call site)
...
8. cfg_utility 'scripts_build_programme_prose_extract' (scripts\build_programme_prose_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
9. cfg_utility 'scripts_export_prose_chapter_edit' (scripts\export_prose_chapter_edit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
10. cfg_utility 'scripts_import_prose_chapter_edit' (scripts\import_prose_chapter_edit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
11. cfg_utility 'scripts_search_prose' (scripts\search_prose.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
...
**408** registered module(s) — **27** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **357** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
| scripts_export_prose_chapter_edit | scripts/export_prose_chapter_edit.py | Superseded by iba/app/lib/prosestore.py (escalation #784) -- logic now lives there, exercised via prose.export_chapter (Prose.ps1 -Step ExportChapter). Kept as the documented CLI entry point, reactivated (escalation #829). | ✓ |  |  |
...
### `escalation.history`
**Escalation deep history** — output `md` · naming `stable` · archived to `archive/` · ToC on
work package `escalation-reporting` → `iba/app/ps/Escalation.ps1` (chained=0)

| # | section | heading | toc label | in ToC |
| --- | --- | --- | --- | --- |
| 0 | item_history | ## #<id> — <short_description> | Item history | ✓ |
| 1 | downward_chain | **downward chain (spawned from #<id>)** | Downward chain | ✓ |

### `escalation.list`
**Open escalations** — output `md+csv` · naming `stable` · archived to `archive/` · ToC on
work package `escalation-reporting` → `iba/app/ps/Escalation.ps1` (chained=0)
...
| 1 | cycle | ## Cycle | Cycle | ✓ |
| 2 | dangling | ## Dangling | Dangling | ✓ |
| 3 | mismatched_pairing | ## Mismatched pairing | Mismatched pairing | ✓ |
| 4 | missing_link | ## Missing link | Missing link | ✓ |
| 5 | incoherent_link | ## Incoherent link | Incoherent link | ✓ |
