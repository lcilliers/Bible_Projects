...
| generated_at | 2026-08-28T05:40:43Z |
...
1. cfg_enum group 'folder_purpose_status' (not looked up by name at runtime — no cfg.enum('folder_purpose_status') or cfg_enum WHERE name='folder_purpose_status' call site)
2. cfg_enum group 'folder_purpose_type' (not looked up by name at runtime — no cfg.enum('folder_purpose_type') or cfg_enum WHERE name='folder_purpose_type' call site)
...
**412** registered module(s) — **31** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **360** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
