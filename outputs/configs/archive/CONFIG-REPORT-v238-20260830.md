...
| generated_at | 2026-08-30T04:42:26Z |
...
3. GOVERNANCE.md was last modified 2026-08-30T04:35:28Z, before the newest applied cfg_change_detail row (2026-08-30T04:42:26Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)
...
4. cfg_prose.prose.patch_output_dir = '"Sessions/Patches"' — 'sessions/patches' does not exist as a folder on disk (project-root-relative)
...
6. Catalogue-Report.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\catalogue-report.ps1' in some tab's first rows)

**Escalation.ps1/worksheet drift** (1) — an Escalation.ps1 parameter not used as a -Flag header anywhere in governance.escalation_worksheet_path:
7. Escalation.ps1 has parameter(s) ['NeedsFollowup', 'ShortDescription', 'Source', 'Tried'] not used as a -Flag header anywhere in escalation actions worksheet.xlsx — the researcher's model sheet may need updating
