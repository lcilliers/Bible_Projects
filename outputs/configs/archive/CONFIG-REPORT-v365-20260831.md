...
| generated_at | 2026-08-31T13:13:14Z |
...
1. cfg_setting 'candidate.quality_report_path' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
2. cfg_setting 'candidate.load_report_path' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
...
3. GOVERNANCE.md was last modified 2026-08-31T10:03:34Z, before the newest applied cfg_change_detail row (2026-08-31T12:49:29Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)
...
4. iba/app/lib/prosestore.py builds a -v{n} filename by hand — no filingkit.versioned_path()/reportkit.oneoff_path() call site in the same file

**PS/worksheet drift** (2) — a script's live param() names not matching its tab's flag headers in governance.ps_worksheet_path:
5. Config-Maintenance.ps1: ps tools worksheet.xlsx tab 'Config-Maintenance' is missing flag column(s) ['Title'] — the script has these parameters now
6. SchemaOverview-BibleResearch-Report.ps1: no tab found in ps tools worksheet.xlsx (expected an embedded path cell 'iba\\app\\ps\\schemaoverview-bibleresearch-report.ps1' in some tab's first rows)
...
**420** registered module(s) — **33** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **365** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
