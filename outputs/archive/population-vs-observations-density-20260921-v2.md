...
Three CSVs, one per grouping level, full disclosure, no truncation: `cluster-population-vs-observations-20260921-v2.csv`, `strong-population-vs-observations-20260921-v2.csv`, `subgroup-population-vs-observations-20260921-v2.csv` (same folder as this report).
...
**M83 / B_intensified_seeking** (Intensive/exhaustive seeking and demanding) is still `answer_complete` while its sibling subgroups in M83 are `ready_for_reading` — confirmed directly via `cluster_subgroup.status`, not inferred from the zero observation counts alone. This subgroup alone accounts for **32 of M83's 496 total verses (6.45%)** and 33 spans (strongs G1567, G1934, H7836) -- meaning M83's current char-reading/char-answers observation counts reflect analysis of only its OTHER, smaller subgroups. Checked the other two clusters for the same pattern: M49, M67 have every subgroup at the same status as its siblings -- this is isolated to M83, not a systemic pattern across all three.
**M83 / C_hostile_pursuit** (Seeking with lethal or hostile intent) is still `answer_complete` while its sibling subgroups in M83 are `ready_for_reading` — confirmed directly via `cluster_subgroup.status`, not inferred from the zero observation counts alone. This subgroup alone accounts for **1 of M83's 496 total verses (0.2%)** and 1 spans (strongs G2614) -- meaning M83's current char-reading/char-answers observation counts reflect analysis of only its OTHER, smaller subgroups. Checked the other two clusters for the same pattern: M49, M67 have every subgroup at the same status as its siblings -- this is isolated to M83, not a systemic pattern across all three.
**M83 / D_consultative_inquiry** (Formal consultation and diligent inquiry) is still `answer_complete` while its sibling subgroups in M83 are `ready_for_reading` — confirmed directly via `cluster_subgroup.status`, not inferred from the zero observation counts alone. This subgroup alone accounts for **152 of M83's 496 total verses (30.65%)** and 164 spans (strongs H1875) -- meaning M83's current char-reading/char-answers observation counts reflect analysis of only its OTHER, smaller subgroups. Checked the other two clusters for the same pattern: M49, M67 have every subgroup at the same status as its siblings -- this is isolated to M83, not a systemic pattern across all three.
