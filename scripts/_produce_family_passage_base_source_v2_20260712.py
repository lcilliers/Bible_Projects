    ...
    doc=OrderedDict([
      ('meta', OrderedDict([
        ('WORK_CONTRACT', OrderedDict([
          ...
          ('output', OrderedDict([
             ('format','A companion JSON per family, keyed to this base source by reading_id so any AI can produce it and it joins back for completeness.'),
             ('file',f'verse-analysis/psalms/_narratives/psalms__{fam}__narratives.json'),
             ...
             ('record_shape',{'reading_id':'str','char_key':'str','ib_char':'str','anchor_ref':'str','passage_ref':'str',
                 'narrative':'the STORY — flowing prose that meets directives 1-10; NOT a per-dimension listing',
                 'citations':'[ve_lexical_id,...] the story rests on (backtrack; directive 7)',
             ...
             ('completeness','Every reading_id in this base source appears exactly once in the output; each narrative cites >=1 ve_lexical_id.'),
             ('destination','DB via output -> patch -> DB (all study work in the DB). The JSON is the transport.'),
          ])),
        ])),
