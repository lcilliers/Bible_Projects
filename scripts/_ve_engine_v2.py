...
def narrate(unit, items):
    ...
    if g("operation"): cl.append("lived out in " + ", ".join(g("operation")))
    if g("quality-bearer"): cl.append("describing " + ", ".join(g("quality-bearer")))
    if g("object"):
        ot = g("object-type")
        cl.append("acting on " + ", ".join(g("object")) + (f' ({ot[0]})' if ot else ""))
    if g("from-source"): cl.append("from " + ", ".join(g("from-source")))
    if g("instrument"): cl.append("by means of " + ", ".join(g("instrument")))
    if g("purpose"): cl.append("in order to " + ", ".join(g("purpose")))
