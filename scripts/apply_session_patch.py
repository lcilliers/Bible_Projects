...
def _apply_operation(conn, op: dict, counts: dict, meta: dict | None = None) -> None:
    ...
    elif table == "prose_section" and operation == "set_status":
        # Reviewer status set/reset (escalation #918, 2026-08-27): a status-only change, no body
        # touched -- the general, either-direction sibling of "approve" (which is approve-only
        # and also stamps approved_at/approved_by). Generated by prosestore.run_set_status.
        target_id = op.get("id") or (op.get("match") or {}).get("id")
        rec = op.get("record") or op.get("values") or {}
        if not target_id:
            raise ValueError(f"{op_id}: prose_section set_status requires id")
        new_status = rec.get("status")
        if not new_status:
            raise ValueError(f"{op_id}: prose_section set_status requires record.status")
        current = conn.execute(
            "SELECT status FROM prose_section WHERE id = ? AND COALESCE(delete_flagged, 0) = 0",
            (target_id,)).fetchone()
        if not current:
            raise ValueError(f"{op_id}: section {target_id} is not an active current prose row")
        snapshot = _prose_section_snapshot(conn, target_id)
        log_id = _write_change_log(conn, "prose_section", target_id, "change", snapshot, meta)
        stamp = _now()
        # approved_by/approved_at only stamped when moving TO approved; reset away from approved
        # clears both, same as leaving an approval behind rather than pretending it still holds.
        approved_by = rec.get("approved_by") if new_status == "approved" else None
        approved_at = stamp if new_status == "approved" else None
        conn.execute(
            "UPDATE prose_section SET status = ?, approved_at = ?, approved_by = ?, "
            "version = ?, updated_at = ? WHERE id = ?",
            (new_status, approved_at, approved_by, log_id, stamp, target_id),
        )
        counts["prose_section_status_set"] = counts.get("prose_section_status_set", 0) + 1
        print(f"  {op_id}: prose_section SET_STATUS id={target_id} -> {new_status!r} v-log={log_id}")
