...
def main():
    ...
    sample = '--sample' in a
    if sample:
        # draw from the fixed acceptance-sample (both books), ordered by seq — the 200 random per dimension
        rows = c.execute("""SELECT si.id sid, v.reference ref, v.chapter, v.verse_num, si.strongs, si.primary_strong,
                  ...
                  CASE WHEN v.book_id=19 THEN 'reread-psalms-2026' ELSE 'reread-proverbs-2026' END sprov, s.seq
               FROM ve_verification_sample s
               JOIN verse_span_index si ON si.id=s.verse_span_id JOIN verse v ON v.id=si.verse_id
               JOIN ve_lexical x ON x.verse_span_id=si.id AND x.ve_nr=s.ve_nr
                    AND x.source_provenance=(CASE WHEN v.book_id=19 THEN 'reread-psalms-2026' ELSE 'reread-proverbs-2026' END)
                    AND x.delete_flagged=0
               WHERE s.ve_nr=? ORDER BY s.seq""", (ve,)).fetchall()
    else:
        rows = c.execute("""SELECT si.id sid, v.reference ref, v.chapter, v.verse_num, si.strongs, si.primary_strong,
                  si.surface, si.morph_code, si.stem, v.verse_text, v.passage_id,
                  x.value stored, x.from_span, x.to_span, x.resolution
