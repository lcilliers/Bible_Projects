# Prose management in IBA — first layer: proposal (escalation #829) — v7
...
> (mutate-in-place, current-state-only) with a new project-wide `record_change_log` table. This
> revision brings v6's build spec **up to date with what #836 actually built**, checked live against
> the DB today, not assumed from the design docs. Nothing in v6's own scope (§4 components I/II/IV/V,
> most of III, §6 D1–D9, §12) is invalidated by #836 — but v6's own §5 III (`prose_section`'s write
> governance) drafted a `supersede-only discipline` rule that #836's build has now **corrected and
> superseded outright** (GOVERNANCE.md §52.2, `cfg_behaviour_rule record-change-log-version-is-
> pointer`), and v6's §1/§1.5/§2 describe the pre-#836 schema, which has changed under it. Every
> section below marked **"v7"** is a real content change verified against live data this round;
> everything else carries forward from v6 unchanged.

**Supersedes v6** (`prose-management-iba-first-layer-proposal-v6-20260824.md`, left on disk for
history, itself superseding v5/v4/v3/v2/v1 — all kept on disk). **Stage:** still
...
only thing that has actually landed** in this whole document's scope so far.
...
unaffected by #836's build** (#836 touched `version`/`updated_at` only). Still a live
`governance.table_columns` violation, still fixed in §5.

**c)** `prose.book_stage_map`'s proposed value — unchanged from v6, unaffected by #836.
...
column` fix widened this round (v7):** still fills the 4 blank `prose_section_type` rows (§1.3b) —
**and now also drops the 3 stale `prose_section` rows** `supersedes_id`/`superseded_by_id`/
`source_file` (§1.3d, new finding).
...
### V — `cfg_column` — **widened this round (v7): fill 4 blank + drop 3 stale**
...
Drop (new this round, §1.3d — logged as a correction against #836's own build sequencing):
...
2. `cfg_column` — fill the 4 blank `prose_section_type` `use` values **and drop the 3 stale
   `prose_section` rows** (§5, component V — *widened, v7*).
...
11. `docs/prose-store-architecture.md` → superseded banner (§8.1, now also citing GOVERNANCE.md §52).
...
Same structure as v6 — unchanged. This revision's own new content (the schema/governance
reconciliation with #836) needs no separate decision — it's a correction to keep the proposal
factually accurate, not a judgement call. If you approve as written, §9's sequence (as revised
above) is what gets submitted.
