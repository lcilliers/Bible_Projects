# Prose management in IBA — first layer: proposal (escalation #829) — v4

**Supersedes v3** (`prose-management-iba-first-layer-proposal-v3-20260823.md`, left on disk for
history, itself superseding v2/v1). This revision records the researcher's decision on v3's §1.4
flag material: the flag-system's *principles* are sound, but application/controls fell apart
project-wide because nothing harnessed them until IBA — *"this lies at the heart of why IBA is
fundamentally important to harness the operations of the project as a whole."* Prose never had
flagging integration historically (confirmed, not a gap to backfill) — but as prose grows into a
large, **authoritative** body of text, flags to signal cross-cutting impact on it *from other
operations* become vital. This is bigger than a prose-specific mechanism, so it now has its own
escalation: **#833 "Flag Management"**, scoped project-wide, kickstarted from v3's §1.4. **#829 is
now formally dependent on #833** for anything flag-related in its own scope — the storage/mechanical
build itself (§4–§9) is unaffected and proceeds independently. §1.4.4 and §7 below are updated to
route to #833 instead of #831 — #831's own "prose-change-flag" scope item is now cross-referenced
to #833 too, so the two escalations don't independently design the same mechanism.
...
┄┄┄┄┄┄┄┄┄ NOT linked to prose_section today — candidate incorporation, §1.4 ┄┄┄┄┄┄┄┄┄

wa_quality_flag_types (29 codes, 7 groups)         wa_session_research_flags (715 rows)
      │  flag_id                                          — has a real resolved lifecycle,
      ▼                                                      but targets word_registry, not
wa_data_quality_flags (19,866 rows,                          prose_section (§1.4.2)
   term/file-scoped, no lifecycle column)
```
...
architecture doc never documents. The quality-flag family (bottom) is drawn separately because
**no FK, junction, or code path connects it to `prose_section` today** — every table in it targets
`word_registry`/`wa_file_index`/a bare term string, never a prose row. §1.4 gives full visibility
into this family so the researcher can assess incorporation directly.
...
### 1.4 The quality-flag family — full visibility (plan/propose in scope here; build is not)

Scope boundary, stated once: **planning/proposing** how a quality-flag-shaped mechanism could
incorporate prose is in scope for #829 — it's a real dependency for #831 (prose add/edit rules,
which claims the prose-change-flag mechanism), and this is the natural place to ground it in the
actual data. **Building** any of it — schema changes, populating a link, wiring a trigger — is not
in #829's own build (§9); whichever shape gets chosen becomes its own build item, home stated in
§1.4.4.

#### 1.4.1 Every flag-related table, not just the 2 named at #784 §7

`wa_quality_flag_types`/`wa_data_quality_flags` were the pair #784 §7 named. Checking the live
catalogue surfaced a wider family — reproduced here in full, since a partial picture would be
exactly the kind of gap this revision exists to close:

| Table | Rows | `cfg_table` state | Role |
|---|---:|---|---|
| `wa_quality_flag_types` | 29 | active | The data-quality/research flag vocabulary — §1.4.2 |
| `wa_data_quality_flags` | 19,866 | **inactive=1** — `cfg_table.use`: *"the table has no resolution or soft-delete column, so it is an append-only log with no lifecycle"* | Instances of the above, raised against a term/file — §1.4.2 |
| `wa_session_research_flags` | 715 | active | A **different, better-shaped** researcher-facing flag queue — genuine `resolved`/`resolved_date`/`resolved_note` lifecycle, routes to a `cluster_link` and a second `cross_registry_id`. Real vocabulary drift (below) but structurally the closest fit to "raised, routed, eventually resolved" — §1.4.3 |
| `phase2_flag_types` | 25 | active | A *different purpose* — analytical tags about a term's own content (e.g. `GOD_AS_SUBJECT`, `SOMATIC_INNER_LINK`), not a "needs attention" flag. Named for completeness, not a candidate. |
| `mti_term_flags` | 1,005 | inactive=1 | Junction, `phase2_flag_types` → `mti_terms`. 994/1,005 rows use just 2 of the 25 codes. Not a candidate — term-scoped, narrow. |
| `wa_term_phase2_flags` | 1,570 | inactive=1 | Same vocabulary, different junction (→ inventory terms). 1,092 rows are one bulk patch. Not a candidate. |
| `wa_flag_type_question_link` | 12 | active | Maps 4 flag types to catalogue questions. Tiny, static, one write in April 2026. Not a candidate on its own, but the *shape* (flag → what it should prompt) is relevant to a "why flagged" design. |

**None of the 7 tables above has any relationship to `prose_section` today** — confirmed by
searching every FK and junction in the schema, not assumed.

#### 1.4.2 `wa_quality_flag_types` / `wa_data_quality_flags` — full content
...
CREATE TABLE wa_quality_flag_types (
    ...
    description TEXT,
    deprecated INTEGER DEFAULT 0, deprecation_note TEXT, category TEXT, research_actions TEXT
);
...
**All 29 flag types**, grouped, with live instance counts (7 columns collapsed to what matters for
assessment — full `description`/`research_actions` text available on request, this is the complete
code list):

| Group | Code | Deprecated | Instances raised |
|---|---|---|---:|
| DATA_COVERAGE | `VERSE_EVIDENCE_MINIMAL` | | 508 |
| DATA_COVERAGE | `THIN_DATA` | **deprecated** → merged into `VERSE_EVIDENCE_CONCENTRATED` | 211 (legacy rows, not repointed) |
| DATA_COVERAGE | `VERSE_EVIDENCE_CONCENTRATED` | | 5,014 |
| DATA_COVERAGE | `NO_WORD_ANALYSIS` | | **7,781** (largest single flag) |
| DATA_COVERAGE | `SPAN_RESOLUTION_CONFLICT` | | 0 |
| DATA_COVERAGE | `SPAN_FILTER_APPLIED` | | 0 |
| DATA_COVERAGE | `VERSE_EVIDENCE_HIGH` | | 278 |
| DATA_COVERAGE | `PROSE_ONLY_MEANING` | | 5,759 |
| DATA_COVERAGE | `CONCRETE_PHYSICAL` | | 315 |
| DATA_QUALITY | `VERSE_EVIDENCE_BREADTH_NOTE` | | 0 |
| DATA_QUALITY | `PH2_DATA_ERROR` | | 0 |
| DATA_QUALITY | `PH2_CROSS_REGISTRY_REQUIRED` | | 0 |
| DATA_QUALITY | `PH2_DATA_QUALITY` | | 0 |
| DATA_QUALITY | `PH2_DATA_SPLIT_REQUIRED` | | 0 |
| DATA_QUALITY | `DATA_INTEGRITY` | | 0 |
| RESEARCHER_DECISION | `PH2_BOUNDARY_QUESTION` | | 0 |
| RESEARCHER_DECISION | `CANDIDATE_REGISTRY_WORD` | | 0 |
| RESEARCHER_DECISION | `RESEARCHER_DECISION` | | 0 |
| SESSION_B | `SB_FINDING` | **deprecated** | 0 |
| SESSION_B | `SB_DIMENSION` | **deprecated** | 0 |
| SESSION_B | `SB_INNER_BEING` | **deprecated** | 0 |
| SESSION_D | `SD_POINTER` | | 0 |
| SESSION_D | `SD_CLUSTER` | | 0 |
| SESSION_D_POINTER | `PH2_CROSS_REF_ENRICHMENT` | | 0 |
| SESSION_D_POINTER | `DIMREVIEW_SESSION_D` | | 0 |
| SESSION_D_POINTER | `THEMATIC_LINK` | | 0 |
| STUDY_REQUIRED | `PH2_THEOLOGICAL_DEPTH_REQUIRED` | | 0 |
| STUDY_REQUIRED | `PH2_EXEGETICAL_STUDY_REQUIRED` | | 0 |
| STUDY_REQUIRED | `PH2_ESCHATOLOGICAL_STUDY_REQUIRED` | | 0 |

**The honest read of this table, stated plainly**: only **7 of 29** declared types have ever been
raised — all 7 in the single `DATA_COVERAGE` group, all engine-computed automatically from STEP
extraction stats (verse count, meaning-block shape), zero human judgement involved. **Every type in
`DATA_QUALITY`, `RESEARCHER_DECISION`, `SESSION_B`, `SESSION_D`, `SESSION_D_POINTER`, and
`STUDY_REQUIRED` — the groups conceptually closest to "this needs attention" — has 0 instances,
ever.** The mechanism's "richer" half is declared but has never actually been exercised. Further,
per `cfg_column`: `last_changed` is populated on only 323/19,866 rows across 4 distinct dates — the
lifecycle-looking column is not actually maintained; the table is genuinely append-only, matching
`cfg_table.use`'s own verdict (§1.4.1). `term_id` mixes identifier schemes (Strong's-like strings,
315 bare integers). `category` (4 non-null values) duplicates `flag_group` exactly where populated.

This is materially thinner than "the closest existing shape" sounded in #784 §7 — the shape exists
in the schema, but only a narrow, fully-automated slice of it has ever run live.

#### 1.4.3 `wa_session_research_flags` — the better-shaped alternative, with its own real problems

```sql
-- (columns, condensed — full cfg_column text available)
id, registry_id (FK word_registry), file_id (FK wa_file_index, NULL >50%),
flag_code, flag_label, strongs_reference, cross_registry_id (FK word_registry),
priority, session_target, description, session_raised, raised_date,
resolved, resolved_date, resolved_note,
cluster_link (comma-separated, not a junction), cluster_link_basis
```

715 rows, 98 distinct `registry_id`s. **This one has what §1.4.2's table lacks**: a genuine
`resolved` boolean (200 closed / 515 open — 72% of the queue still open) with `resolved_date`/
`resolved_note`, and `cross_registry_id` for observations spanning two words. But real problems of
its own, found live: `priority` mixes `MEDIUM`/`HIGH`/`LOW` with an unrelated `normal`/`advisory`
scale (20 rows); `session_target` spells the same 3 destinations 7 ways (`'D'`/`'Session D'`,
`'B'`/`'Session B'`, `'Researcher'`/`'researcher'`) with no CHECK constraint; 447 of 715 rows route
to `'D'`/`'Session D'` — **the abandoned Session D workstream**; `cluster_link` is a
comma-separated string (`'M04,M05'`), not a real junction, so it can't be joined without splitting;
`raised_date`/`resolved_date` mix full ISO timestamps and bare dates. It targets `word_registry`,
never `prose_section` — no more linked to prose today than the other table.

#### 1.4.4 Incorporation options — sketched for assessment, none decided or built

**Superseded framing (v4):** the researcher's decision on this material (§ banner above) widened it
from a prose-specific mechanism to the general project-wide flag mechanism IBA should harness —
prose's growing, authoritative status is the trigger, not the boundary. Escalation **#833 "Flag
Management"** now owns this design, kickstarted from the 3 options below; they are carried forward
here unchanged as that escalation's starting material, not re-scoped to prose-only in place.

Three shapes, none chosen:

1. **Extend `wa_data_quality_flags`/`wa_quality_flag_types`** with a nullable `prose_section_id` FK
   alongside the existing `file_id`, plus a real `resolved`/`resolved_at` pair. Cheapest schema
   change, but inherits the table's own dead weight (22 unused types, no working lifecycle
   precedent to build on — §1.4.2 already shows the "richer" categories were never actually
   exercised for *anything*, not just prose).
2. **Extend `wa_session_research_flags`** with the same nullable `prose_section_id` FK. Better
   lifecycle precedent (a real 200/515 resolved/open split), but inherits its vocabulary drift
   (`priority`, `session_target`) and its `cluster_link` string-not-junction problem — extending a
   table with known data-quality issues onto a new entity risks compounding them.
3. **A new, purpose-built table** — `prose_section_flag` or similar — polymorphic-ready from the
   start (`target_table`/`target_id` rather than a single hardcoded FK, so it isn't prose-only if a
   third target ever appears), a real `raised_at`/`resolved_at`/`resolved_by` lifecycle from day
   one, and a controlled `cfg_enum`-backed vocabulary instead of free text. Clean, but starts from
   zero rather than reusing anything — the option closest to what #784 §7's own emerging design
   ("the prose-change-flag table... is the change engine") already sounded like it was heading
   toward.

No recommendation forced here. Whichever shape is chosen, the **build** is out of #829's own scope
(§9) — home: **#833** ("Flag Management," project-wide scope, not prose-narrowed), which also now
carries the trigger question (what causes a flag to be raised) as part of its own remit rather than
splitting it from #831's add/edit rules.
