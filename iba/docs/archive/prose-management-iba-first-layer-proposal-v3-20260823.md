# Prose management in IBA — first layer: proposal (escalation #829) — v3

**Supersedes v2** (`prose-management-iba-first-layer-proposal-v2-20260823.md`, left on disk for
history, itself superseding v1). This revision answers the researcher's follow-up review of v2:
§1.1's relationship picture must include the quality-flag tables; full visibility (schema + content)
of the current quality-flag tables, so the researcher can assess incorporation directly, not from a
narrative summary; and a scope clarification — **planning/proposing** how quality flags incorporate
into prose is in scope for #829 (this document); **building** that incorporation is not. §0 below
maps this and every earlier review point to where it's answered.

**Why this matters now, not later** (researcher, verbatim): *"the next escalation (edit/update) will
depend on it"* — #831 (prose add/edit operational rules) explicitly includes the prose-change-flag
mechanism in its own scope, and that design needs the real shape of what exists today, not the
narrative-only assessment §1.4 gave in v2.
...
| **v3 (this revision):** §1.1 must include quality flags | §1.1 — the full flag-table family now in the relationship picture |
...
No recommendation forced here — this is the researcher's own call to make with the real data now
visible (§1.4.1–1.4.3), not a design this proposal should pre-empt. Whichever shape is chosen, the
**build** is out of #829's own scope (§9) — home: **#831**, since the mechanism's actual trigger
(what causes a flag to be raised when prose needs attention) is inseparable from the add/edit rules
already being designed there.
...
| Quality-flag incorporation shape — which of §1.4.4's 3 options (or another), and its build | **#831** — full visibility given (§1.4.1–1.4.3), option is the researcher's own call, not pre-empted here |
