# WA — Verse Reading Technique: Base Lexical Reading (v4)

**Filename:** WA-verse-reading-technique-v4-2026-08-05.md
**Date timestamp:** 2026-08-05
**Previous outputs referenced:** supersedes `WA-verse-reading-technique-v1, V2 and V3 and also various prototyping documents


**Version:** 4 — **draft** The researcher manually rewritten this version from other documents to consolidate and normalize the different guidances and instructions. It was necessary to reframe previous versions to be more explicit about the reading of the verse lexical and the inner being relevance, and to separate the two. The previous versions were not clear enough about what was being read, and what was not being read, and what was being assumed. This version is a complete rewrite of the previous versions, with a focus on clarity and explicitness.

## Why this exists, and what it does not cover

This document is the canonical instruction for verse reading to create the base lexical reading of a verse. covers one thing: reading a verse's clauses closely enough, from their actual lexical and grammatical data, to establish what each clause actually says — before any question of inner-being phenomena, states, or movements is asked. Then key elements of inner being related context are debated based on the lexical understanding of the verse context. 

This instruction is there to normalise verse reading in a fix pattern that can easily be accessed via the database, to contain verse context drift by being very specific on what should not be skipped, to enforce reading the lexical meaning of each word and not just the standard english translations from the verse table, to allow for anchoring meaning to the actual context of the verse, to allow matching and searching on verse contents in the DB via the verse lexical. 

## The reading method

Each verse is read as a unit. When verses are introduced as a batch, a passage or a group, then each verse have the same reading and analysis weight. T1-T9 must be applied to each verse in equal proportion. Verse reading is not pattern recognition, it is about the meaning in context of each element. The lexical description of the verse T1-T5 must be completed before the inner being elements T6-T9 is derived.
...
(verified 2026-08-03 direct against `iba/app/db/iba.db`):
