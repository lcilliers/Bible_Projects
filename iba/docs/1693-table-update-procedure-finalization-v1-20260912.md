...
|---|---|
| `cluster_subgroup` | assigns `id`; resolves strongs against `iba.strong` (was `mti_terms.id`, changed 2026-09-13 DB fork — #1690 §2.2); writes the `FLAG` signpost as a same-cluster subgroup (confirmed, #1690 §3 item 3); **NEW, 2026-09-16 (#1526, #1690 §2A):** resolves `anchor_verse_reference` fresh against `iba.db.verse.reference`, same pattern as `ib_node.verse_reference` above, in the SAME write as the rest of the subgroup row — never a separate pass, and never left null-then-backfilled on a broaden (§3) |
