# Prose management in IBA — first layer: proposal (escalation #829) — v5

> **Superseded by v6** (`prose-management-iba-first-layer-proposal-v6-20260824.md`) — left on disk
> for history, per this series' own convention. v6 fixes §6 D5's text (now matches D6) and adds the
> on-hold banner (#829 held pending escalation #836).

**Supersedes v4** (`prose-management-iba-first-layer-proposal-v4-20260823.md`, left on disk for
history, itself superseding v3/v2/v1). Sections 1–11 below are **unchanged from v4** — the
...
| **D5** | `cluster_code` — add a real FK to `cluster(cluster_code)` via table-rebuild migration (0 live orphans, confirmed). Tension flagged 2026-08-24 (citation-column framing vs. hardening now) — see §1.1. | **Not addressed in the researcher's 2026-08-24 D1/D2/D3/D4/D6/D7 review round (D5 wasn't named).** Left genuinely open rather than assumed to follow D3/D4/D6's pattern — asking directly rather than guessing. | **#832**, if not approved now |
