...
| generated_at | 2026-08-24T09:59:55Z |
...
1. cfg_enum group 'record_change_log_change_type' (not looked up by name at runtime — no cfg.enum('record_change_log_change_type') or cfg_enum WHERE name='record_change_log_change_type' call site)
2. cfg_enum group 'record_change_log_status' (not looked up by name at runtime — no cfg.enum('record_change_log_status') or cfg_enum WHERE name='record_change_log_status' call site)
...
**407** registered module(s) — **27** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **360** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
