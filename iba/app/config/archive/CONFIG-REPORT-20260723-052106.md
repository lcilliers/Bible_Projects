...
| generated_at | 2026-07-22T19:47:20Z |
...
- cfg_setting 'governance.build_md_on_code_change'
- cfg_setting 'governance.governance_md_on_rule_change'
- cfg_enum group 'escalation_answer'

**Settings needing justification** (7) — module already has its own dedicated table:
- cfg_setting 'candidate.lemma_base_pattern' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
- cfg_setting 'candidate.tag_clean_pattern' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
- cfg_setting 'candidate.quality_report_path' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
- cfg_setting 'candidate.concept_delimiter_pattern' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
- cfg_setting 'candidate.tag_max_words' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
- cfg_setting 'candidate.transliteration_pattern' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
- cfg_setting 'candidate.load_report_path' (module 'candidate') — candidate already has its own dedicated table (cfg_candidate_rule); confirm this belongs in shared cfg_setting rather than there
...
| report | report.output_dir | iba/app | where report.word writes its output |
...
| table_export | table_export.output_dir | iba/app/export | where table.export writes its CSVs |
...
**candidate-quality** — runs over `none` · script `Candidate-Quality.ps1`
| # | step | handler | scope | does |
| --- | --- | --- | --- | --- |
...
**configuration-maintenance** — runs over `none` · script `Config-Maintenance.ps1`
| # | step | handler | scope | does |
| --- | --- | --- | --- | --- |
...
**passage-quality** — runs over `none` · script `Passage-Quality.ps1`
| # | step | handler | scope | does |
| --- | --- | --- | --- | --- |
...
**reports** — runs over `none` · script `Reports.ps1`
| # | step | handler | scope | does |
| --- | --- | --- | --- | --- |
...
**10 of 30 conditions ESCALATE** (pause-continue — the researcher is asked); the rest either stop the run outright (report-stop) or continue with a logged warning (report-continue). Per the researcher's 2026-07-21 rule: any finding that needs a judgement call must be in the first group, not silently in the second or third.
...
### span
_ONE ROW PER CODE of a verse (O3) — particles get their own row_ — L4a — SOURCE, immutable. A parse of verse.preview. position is the running code index; (verse, position) is the key.
...
| strong_variant | TEXT |  |  |  | strong.strongNumber | ONE strong code | parse:verse.preview |
| morph_code | TEXT |  |  |  |  | the grammatical layer — one, aligned with the code | parse:verse.preview |
| is_particle | INTEGER |  |  |  |  | 1 if a grammar-particle code (H9xxx/G9xxx) | derived:strong_variant |
...
| config_module | registry, raw, step, report, candidate, passage, configmaint, validation, governance, retention, notification, table_export |
...
work package `candidate-quality` → `Candidate-Quality.ps1` (chained=0)
...
work package `configuration-maintenance` → `Config-Maintenance.ps1` (chained=0)
...
work package `passage-quality` → `Passage-Quality.ps1` (chained=0)
...
work package `reports` → `Reports.ps1` (chained=0)
