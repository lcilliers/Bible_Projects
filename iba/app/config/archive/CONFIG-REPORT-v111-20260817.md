...
| generated_at | 2026-08-17T10:37:40Z |
...
7. GOVERNANCE.md was last modified 2026-08-17T03:35:32Z, before the newest applied cfg_change_detail row (2026-08-17T09:31:47Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)

**Unregistered lib modules** (0) — iba/app/lib/*.py with no cfg_utility row:
_(none)_

**Low config-density utilities** (213) — NON-EXEMPT cfg_utility module with zero real Cfg-method call sites of its own (see §2 Utilities registry for the full module list, including the 11 already declared config_exempt):
...
22. cfg_utility 'iba_prototype_export_md' (iba\prototype\export_md.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
23. cfg_utility 'iba_prototype_inspect_verse' (iba\prototype\inspect_verse.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
24. cfg_utility 'iba_scripts_build_dbschema' (iba\scripts\build_dbschema.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
25. cfg_utility 'iba_scripts_cfg_apply' (iba\scripts\cfg_apply.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
26. cfg_utility 'iba_scripts_cfg_helper' (iba\scripts\cfg_helper.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
27. cfg_utility 'iba_scripts_cfg_kernel' (iba\scripts\cfg_kernel.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
28. cfg_utility 'iba_scripts_probe_step_api' (iba\scripts\probe_step_api.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
29. cfg_utility 'query_db' (query_db.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
30. cfg_utility 'research_VE_lexical_faculty_map_build_build_batch4' (research\VE-lexical\faculty-map-build\_build_batch4.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
31. cfg_utility 'research_VE_lexical_faculty_map_build_classify_batch1' (research\VE-lexical\faculty-map-build\_classify_batch1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
32. cfg_utility 'scripts_apply_cause_from_api' (scripts\_apply_cause_from_api.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
33. cfg_utility 'scripts_apply_create_constitution_cluster' (scripts\_apply_create_constitution_cluster.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
34. cfg_utility 'scripts_apply_create_vc_for_onboarded' (scripts\_apply_create_vc_for_onboarded.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
35. cfg_utility 'scripts_apply_d6_capture_contributor_source' (scripts\_apply_d6_capture_contributor_source.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
36. cfg_utility 'scripts_apply_descriptions_patch' (scripts\_apply_descriptions_patch.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
37. cfg_utility 'scripts_apply_drop_code_softdelete' (scripts\_apply_drop_code_softdelete.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
38. cfg_utility 'scripts_apply_excluded_registry_cascade' (scripts\_apply_excluded_registry_cascade.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
39. cfg_utility 'scripts_apply_faculty_rederive_v1' (scripts\_apply_faculty_rederive_v1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
40. cfg_utility 'scripts_apply_field_from_api' (scripts\_apply_field_from_api.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
41. cfg_utility 'scripts_apply_flag_empty_to_t2' (scripts\_apply_flag_empty_to_t2.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
42. cfg_utility 'scripts_apply_flag_triage_moves' (scripts\_apply_flag_triage_moves.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
43. cfg_utility 'scripts_apply_generate_ve_lexical_v2' (scripts\_apply_generate_ve_lexical_v2.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
44. cfg_utility 'scripts_apply_ingest_verse_morphology' (scripts\_apply_ingest_verse_morphology.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
45. cfg_utility 'scripts_apply_l2_rollup' (scripts\_apply_l2_rollup.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
46. cfg_utility 'scripts_apply_l2_write' (scripts\_apply_l2_write.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
47. cfg_utility 'scripts_apply_l2_write_refit' (scripts\_apply_l2_write_refit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
48. cfg_utility 'scripts_apply_language_reconcile' (scripts\_apply_language_reconcile.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
49. cfg_utility 'scripts_apply_link_mti_term_id' (scripts\_apply_link_mti_term_id.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
50. cfg_utility 'scripts_apply_migrate_sb_findings' (scripts\_apply_migrate_sb_findings.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
51. cfg_utility 'scripts_apply_migrate_ve_findings_to_lexical' (scripts\_apply_migrate_ve_findings_to_lexical.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
52. cfg_utility 'scripts_apply_morph_backfill' (scripts\_apply_morph_backfill.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
53. cfg_utility 'scripts_apply_persist_narration_finding_v1' (scripts\_apply_persist_narration_finding_v1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
54. cfg_utility 'scripts_apply_phase2_flags_patch' (scripts\_apply_phase2_flags_patch.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
55. cfg_utility 'scripts_apply_prose_programme_chapter01' (scripts\_apply_prose_programme_chapter01.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
56. cfg_utility 'scripts_apply_registry_metadata_patch' (scripts\_apply_registry_metadata_patch.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
57. cfg_utility 'scripts_apply_reset_l2_meaning_flags' (scripts\_apply_reset_l2_meaning_flags.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
58. cfg_utility 'scripts_apply_sense_from_subgloss' (scripts\_apply_sense_from_subgloss.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
59. cfg_utility 'scripts_apply_softdelete_excluded_empty_terms' (scripts\_apply_softdelete_excluded_empty_terms.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
60. cfg_utility 'scripts_apply_softdelete_orphan_verses' (scripts\_apply_softdelete_orphan_verses.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
61. cfg_utility 'scripts_apply_stem_patch' (scripts\_apply_stem_patch.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
62. cfg_utility 'scripts_apply_supersede_old_mechanical' (scripts\_apply_supersede_old_mechanical.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
63. cfg_utility 'scripts_apply_t2_soft_delete' (scripts\_apply_t2_soft_delete.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
64. cfg_utility 'scripts_apply_term_decisions' (scripts\_apply_term_decisions.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
65. cfg_utility 'scripts_apply_ve_rebuild_mechanical_v1' (scripts\_apply_ve_rebuild_mechanical_v1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
66. cfg_utility 'scripts_apply_verse_read_meaning' (scripts\_apply_verse_read_meaning.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
67. cfg_utility 'scripts_apply_verse_uniqueness_cleanup' (scripts\_apply_verse_uniqueness_cleanup.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
68. cfg_utility 'scripts_apply_wipe_ve_lexical_v1' (scripts\_apply_wipe_ve_lexical_v1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
69. cfg_utility 'scripts_assess_cluster_profiles' (scripts\_assess_cluster_profiles.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
70. cfg_utility 'scripts_assess_cluster_v3_2_preeval' (scripts\_assess_cluster_v3_2_preeval.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
71. cfg_utility 'scripts_assess_corpus_keyword_map' (scripts\_assess_corpus_keyword_map.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
72. cfg_utility 'scripts_assess_corpus_keyword_typed' (scripts\_assess_corpus_keyword_typed.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
73. cfg_utility 'scripts_assess_cross_cluster_cooccurrence' (scripts\_assess_cross_cluster_cooccurrence.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
74. cfg_utility 'scripts_assess_keyword_corpus_report' (scripts\_assess_keyword_corpus_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
75. cfg_utility 'scripts_assess_keyword_overlap' (scripts\_assess_keyword_overlap.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
76. cfg_utility 'scripts_assess_l2_findings_view' (scripts\_assess_l2_findings_view.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
77. cfg_utility 'scripts_assess_l2_triage' (scripts\_assess_l2_triage.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
78. cfg_utility 'scripts_assess_link_correlation' (scripts\_assess_link_correlation.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
79. cfg_utility 'scripts_assess_meaning_tables' (scripts\_assess_meaning_tables.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
80. cfg_utility 'scripts_assess_mti_duplicate_terms' (scripts\_assess_mti_duplicate_terms.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
81. cfg_utility 'scripts_assess_p2_verse_scenarios' (scripts\_assess_p2_verse_scenarios.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
82. cfg_utility 'scripts_assess_qa_method_effectiveness' (scripts\_assess_qa_method_effectiveness.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
83. cfg_utility 'scripts_assess_qa_method_quality_review' (scripts\_assess_qa_method_quality_review.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
84. cfg_utility 'scripts_assess_read_dedup' (scripts\_assess_read_dedup.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
85. cfg_utility 'scripts_assess_registry_grounding' (scripts\_assess_registry_grounding.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
86. cfg_utility 'scripts_assess_registry_vs_keywords' (scripts\_assess_registry_vs_keywords.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
87. cfg_utility 'scripts_assess_relationship_probe' (scripts\_assess_relationship_probe.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
88. cfg_utility 'scripts_assess_shared_forms' (scripts\_assess_shared_forms.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
89. cfg_utility 'scripts_assess_study_state' (scripts\_assess_study_state.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
90. cfg_utility 'scripts_assess_t2_cleanup' (scripts\_assess_t2_cleanup.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
91. cfg_utility 'scripts_assess_t2_relevance_surface' (scripts\_assess_t2_relevance_surface.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
92. cfg_utility 'scripts_assess_termsense_ranking' (scripts\_assess_termsense_ranking.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
93. cfg_utility 'scripts_assess_verse_assembly' (scripts\_assess_verse_assembly.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
94. cfg_utility 'scripts_assess_verse_corroboration' (scripts\_assess_verse_corroboration.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
95. cfg_utility 'scripts_assess_verse_raw_data' (scripts\_assess_verse_raw_data.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
96. cfg_utility 'scripts_audit_step_extract_archiving' (scripts\_audit_step_extract_archiving.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
97. cfg_utility 'scripts_backfill_span_match' (scripts\_backfill_span_match.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
98. cfg_utility 'scripts_batch_audit' (scripts\_batch_audit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
99. cfg_utility 'scripts_batch_extract' (scripts\_batch_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
100. cfg_utility 'scripts_build_M01_verse_read_review' (scripts\_build_M01_verse_read_review.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
101. cfg_utility 'scripts_build_cluster_verse_read_gate' (scripts\_build_cluster_verse_read_gate.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
102. cfg_utility 'scripts_build_ps119' (scripts\_build_ps119.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
103. cfg_utility 'scripts_build_t2_flag_sample' (scripts\_build_t2_flag_sample.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
104. cfg_utility 'scripts_build_term_verse_findings_report' (scripts\_build_term_verse_findings_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
105. cfg_utility 'scripts_build_vc_batch' (scripts\_build_vc_batch.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
106. cfg_utility 'scripts_build_vc_revision_ledger' (scripts\_build_vc_revision_ledger.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
107. cfg_utility 'scripts_build_verse_read_pilot_review' (scripts\_build_verse_read_pilot_review.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
108. cfg_utility 'scripts_cc_verse_read' (scripts\_cc_verse_read.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
109. cfg_utility 'scripts_check_doc_versions' (scripts\_check_doc_versions.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
110. cfg_utility 'scripts_check_fi_ti_chain' (scripts\_check_fi_ti_chain.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
111. cfg_utility 'scripts_check_integrity_controls' (scripts\_check_integrity_controls.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
112. cfg_utility 'scripts_check_mti_terms' (scripts\_check_mti_terms.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
113. cfg_utility 'scripts_check_softdelete_integrity' (scripts\_check_softdelete_integrity.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
114. cfg_utility 'scripts_check_ve_seat_completeness' (scripts\_check_ve_seat_completeness.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
115. cfg_utility 'scripts_check_ve_signal_lists' (scripts\_check_ve_signal_lists.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
116. cfg_utility 'scripts_db_introspect' (scripts\_db_introspect.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
117. cfg_utility 'scripts_delete_empty_fi' (scripts\_delete_empty_fi.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
118. cfg_utility 'scripts_discover_word_terms' (scripts\_discover_word_terms.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
119. cfg_utility 'scripts_explore_cluster_timing' (scripts\_explore_cluster_timing.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
120. cfg_utility 'scripts_explore_drop_code_findings' (scripts\_explore_drop_code_findings.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
121. cfg_utility 'scripts_explore_m_vs_r_divergence' (scripts\_explore_m_vs_r_divergence.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
122. cfg_utility 'scripts_explore_tier_findings' (scripts\_explore_tier_findings.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
123. cfg_utility 'scripts_explore_ve_by_cluster' (scripts\_explore_ve_by_cluster.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
124. cfg_utility 'scripts_generate_cluster_gate' (scripts\_generate_cluster_gate.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
125. cfg_utility 'scripts_generate_dimension_report' (scripts\_generate_dimension_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
126. cfg_utility 'scripts_generate_meaning_quality_check' (scripts\_generate_meaning_quality_check.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
127. cfg_utility 'scripts_generate_programme_report' (scripts\_generate_programme_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
128. cfg_utility 'scripts_generate_verse_meanings_export' (scripts\_generate_verse_meanings_export.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
129. cfg_utility 'scripts_integrity_full_check' (scripts\_integrity_full_check.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
130. cfg_utility 'scripts_list_shared_words' (scripts\_list_shared_words.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
131. cfg_utility 'scripts_patch_report' (scripts\_patch_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
132. cfg_utility 'scripts_preflight_m20_dir_005_M20_A_mapping' (scripts\_preflight_m20_dir_005_M20_A_mapping.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
133. cfg_utility 'scripts_pro_read_lib' (scripts\_pro_read_lib.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
134. cfg_utility 'scripts_produce_final_extract' (scripts\_produce_final_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
135. cfg_utility 'scripts_produce_registry_full_extract' (scripts\_produce_registry_full_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
136. cfg_utility 'scripts_produce_vc_word_report' (scripts\_produce_vc_word_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
137. cfg_utility 'scripts_produce_ve_narration_v1' (scripts\_produce_ve_narration_v1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
138. cfg_utility 'scripts_prototype_finding_lifecycle' (scripts\_prototype_finding_lifecycle.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
139. cfg_utility 'scripts_prototype_l1_mechanical' (scripts\_prototype_l1_mechanical.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
140. cfg_utility 'scripts_prototype_l1_morph' (scripts\_prototype_l1_morph.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
141. cfg_utility 'scripts_prototype_meaning_run' (scripts\_prototype_meaning_run.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
142. cfg_utility 'scripts_prototype_p1_keywords' (scripts\_prototype_p1_keywords.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
143. cfg_utility 'scripts_prototype_step_morph' (scripts\_prototype_step_morph.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
144. cfg_utility 'scripts_realign_meaning_tables' (scripts\_realign_meaning_tables.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
145. cfg_utility 'scripts_realign_quality_flags' (scripts\_realign_quality_flags.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
146. cfg_utility 'scripts_repair_02_zero_padding' (scripts\_repair_02_zero_padding.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
147. cfg_utility 'scripts_repair_03_wa_file_index' (scripts\_repair_03_wa_file_index.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
148. cfg_utility 'scripts_repair_05_wa_term_related_words' (scripts\_repair_05_wa_term_related_words.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
149. cfg_utility 'scripts_repair_06_wa_term_root_family' (scripts\_repair_06_wa_term_root_family.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
150. cfg_utility 'scripts_repair_07_wa_verse_records' (scripts\_repair_07_wa_verse_records.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
151. cfg_utility 'scripts_reread_ledger_lib' (scripts\_reread_ledger_lib.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
152. cfg_utility 'scripts_reset_registry_status' (scripts\_reset_registry_status.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
153. cfg_utility 'scripts_run_cause_api' (scripts\_run_cause_api.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
154. cfg_utility 'scripts_run_ve_reads_governed' (scripts\_run_ve_reads_governed.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
155. cfg_utility 'scripts_schema_dump' (scripts\_schema_dump.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
156. cfg_utility 'scripts_term_sharing_spider' (scripts\_term_sharing_spider.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
157. cfg_utility 'scripts_tmp_read_cycle2_rest' (scripts\_tmp_read_cycle2_rest.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
158. cfg_utility 'scripts_tmp_read_cycle3' (scripts\_tmp_read_cycle3.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
159. cfg_utility 'scripts_update_claude_code_instructions' (scripts\_update_claude_code_instructions.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
160. cfg_utility 'scripts_update_reference_doc' (scripts\_update_reference_doc.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
161. cfg_utility 'scripts_update_registry_guide' (scripts\_update_registry_guide.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
162. cfg_utility 'scripts_ve_engine_v2' (scripts\_ve_engine_v2.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
163. cfg_utility 'scripts_analytics_bible_analytics' (scripts\analytics\bible_analytics.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
164. cfg_utility 'scripts_analytics_db_client' (scripts\analytics\db_client.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
165. cfg_utility 'scripts_analytics_morph_util' (scripts\analytics\morph_util.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
166. cfg_utility 'scripts_analytics_step_client' (scripts\analytics\step_client.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
167. cfg_utility 'scripts_analytics_word_export' (scripts\analytics\word_export.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
168. cfg_utility 'scripts_analytics_zotero_client' (scripts\analytics\zotero_client.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
169. cfg_utility 'scripts_apply_session_patch' (scripts\apply_session_patch.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
170. cfg_utility 'scripts_backfill_root_families' (scripts\backfill_root_families.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
171. cfg_utility 'scripts_backup_db_to_nas' (scripts\backup_db_to_nas.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
172. cfg_utility 'scripts_build_cause_api_package' (scripts\build_cause_api_package.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
173. cfg_utility 'scripts_build_cluster_findings_digest' (scripts\build_cluster_findings_digest.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
174. cfg_utility 'scripts_build_complete_extract' (scripts\build_complete_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
175. cfg_utility 'scripts_build_corpus_prose' (scripts\build_corpus_prose.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
176. cfg_utility 'scripts_build_correlation_extract' (scripts\build_correlation_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
177. cfg_utility 'scripts_build_dimension_extract' (scripts\build_dimension_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
178. cfg_utility 'scripts_build_field_api_package' (scripts\build_field_api_package.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
179. cfg_utility 'scripts_build_file_manifest' (scripts\build_file_manifest.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
180. cfg_utility 'scripts_build_file_patterns_extract' (scripts\build_file_patterns_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
181. cfg_utility 'scripts_build_label_patterns_extract' (scripts\build_label_patterns_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
182. cfg_utility 'scripts_build_m01_by_characteristic' (scripts\build_m01_by_characteristic.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
183. cfg_utility 'scripts_build_m01_findings_oldnew_extract' (scripts\build_m01_findings_oldnew_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
184. cfg_utility 'scripts_build_m02_findings_oldnew_extract' (scripts\build_m02_findings_oldnew_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
185. cfg_utility 'scripts_build_obs_catalogue_export' (scripts\build_obs_catalogue_export.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
186. cfg_utility 'scripts_build_obs_catalogue_tiered_extract' (scripts\build_obs_catalogue_tiered_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
187. cfg_utility 'scripts_build_patch_types_extract' (scripts\build_patch_types_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
188. cfg_utility 'scripts_build_programme_prose_extract' (scripts\build_programme_prose_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
189. cfg_utility 'scripts_build_reference_snapshot' (scripts\build_reference_snapshot.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
190. cfg_utility 'scripts_build_rules_extract' (scripts\build_rules_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
191. cfg_utility 'scripts_build_script_registry' (scripts\build_script_registry.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
192. cfg_utility 'scripts_build_session_a_prose' (scripts\build_session_a_prose.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
193. cfg_utility 'scripts_build_ve_lexical_extract' (scripts\build_ve_lexical_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
194. cfg_utility 'scripts_build_vocab_extract' (scripts\build_vocab_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
195. cfg_utility 'scripts_build_word_relationship_report' (scripts\build_word_relationship_report.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
196. cfg_utility 'scripts_classify_term_introduction_source' (scripts\classify_term_introduction_source.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
197. cfg_utility 'scripts_combine_cluster_published_to_docx' (scripts\combine_cluster_published_to_docx.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
198. cfg_utility 'scripts_cost_ledger' (scripts\cost_ledger.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
199. cfg_utility 'scripts_export_database_schema' (scripts\export_database_schema.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
200. cfg_utility 'scripts_export_prose_chapter_edit' (scripts\export_prose_chapter_edit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
201. cfg_utility 'scripts_export_tier_catalogue' (scripts\export_tier_catalogue.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
202. cfg_utility 'scripts_export_ve_status_reports' (scripts\export_ve_status_reports.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
203. cfg_utility 'scripts_export_word_json' (scripts\export_word_json.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
204. cfg_utility 'scripts_extract_term_data' (scripts\extract_term_data.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
205. cfg_utility 'scripts_generate_programme_snapshot' (scripts\generate_programme_snapshot.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
206. cfg_utility 'scripts_generate_registry_overview' (scripts\generate_registry_overview.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
207. cfg_utility 'scripts_generate_session_a_extract' (scripts\generate_session_a_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
208. cfg_utility 'scripts_import_prose_chapter_edit' (scripts\import_prose_chapter_edit.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
209. cfg_utility 'scripts_inspect_db_only_terms' (scripts\inspect_db_only_terms.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
210. cfg_utility 'scripts_list_tables' (scripts\list_tables.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
211. cfg_utility 'scripts_populate_dimension_index' (scripts\populate_dimension_index.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
212. cfg_utility 'scripts_query_h2734' (scripts\query_h2734.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
213. cfg_utility 'scripts_readiness_sweep_pilot' (scripts\readiness_sweep_pilot.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
214. cfg_utility 'scripts_readiness_sweep_programme_scan' (scripts\readiness_sweep_programme_scan.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
215. cfg_utility 'scripts_search_prose' (scripts\search_prose.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
216. cfg_utility 'scripts_token_cost_history' (scripts\token_cost_history.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
217. cfg_utility 'scripts_v3_2_l1' (scripts\v3_2_l1.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
218. cfg_utility 'scripts_verify_soul' (scripts\verify_soul.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
219. cfg_utility 'scripts_verse_vertical_pass' (scripts\verse_vertical_pass.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
220. cfg_utility 'scripts_word_study_extract' (scripts\word_study_extract.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
...
**391** registered module(s) — **16** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **143** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
