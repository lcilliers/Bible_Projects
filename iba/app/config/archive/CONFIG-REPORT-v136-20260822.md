...
| generated_at | 2026-08-22T09:54:25Z |
...
1. cfg_setting 'raw.zero_strongs_action' (key not found together with a cfg.setting(...) call in any one file)
2. cfg_enum group 'resolution_kind' (not looked up by name at runtime — no cfg.enum('resolution_kind') or cfg_enum WHERE name='resolution_kind' call site)
...
3. GOVERNANCE.md was last modified 2026-08-21T16:05:00Z, before the newest applied cfg_change_detail row (2026-08-22T05:14:04Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)

**Unregistered lib modules** (1) — iba/app/lib/*.py with no cfg_utility row:
4. iba/app/lib/prosestore.py has no cfg_utility row — run migration/bootstrap_cfg_utility.py to register it
...
**401** registered module(s) — **24** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **358** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
| scripts_export_prose_chapter_edit | scripts/export_prose_chapter_edit.py | Export one current prose chapter or section as a temporary editable Markdown file. -- INACTIVE 2026-08-18 (escalation #729): zero Cfg-method call sites, researcher decision ("set these 110 module to inactive; if the time arise when they need to be used, then the script can be updated to be fully compliant") rather than config_exempt=1. |  |  |  |
...
| escalation | 1 | in-progress | either party directly (Update state=in-progress); OR system: next_action=revise |
...
| escalation | 5 | withdraw | party's explicit choice at Update: next_action=reject, state=withdraw |
| escalation | 6 | supersede | party's explicit choice at Update: next_action=reject, state=supersede |
| escalation | 7 | completed | system: manual next_action=approved+resolution present; OR: dispatcher-tied default (answer_for_run, decision not hold/noted) |
