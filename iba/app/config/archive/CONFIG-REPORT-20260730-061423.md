...
| generated_at | 2026-07-30T05:03:13Z |
...
_Computed fresh on every regenerate — the full detail behind `configmaint.validate`'s escalation, which references this section by path rather than repeating it. Not errors — advisory. See GOVERNANCE.md §5B._
...
- GOVERNANCE.md was last modified 2026-07-30T03:42:35Z, before the newest applied cfg_change_detail row (2026-07-30T05:03:13Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)

**Unregistered lib modules** (0) — iba/app/lib/*.py with no cfg_utility row:
_(none)_

**Low config-density utilities** (13) — cfg_utility module with zero cfg.setting()/cfg.enum() call sites in its own file:
- cfg_utility 'cfg' (iba\app\lib\cfg.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'cfgcheck' (iba\app\lib\cfgcheck.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'cfgload' (iba\app\lib\cfgload.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'db' (iba\app\lib\db.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'dbsnapshot' (iba\app\lib\dbsnapshot.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'lexiconparse' (iba\app\lib\lexiconparse.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'passagetrack' (iba\app\lib\passagetrack.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'registryreport' (iba\app\lib\registryreport.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'retention' (iba\app\lib\retention.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'schemareport' (iba\app\lib\schemareport.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'seedreport' (iba\app\lib\seedreport.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'spanreport' (iba\app\lib\spanreport.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
- cfg_utility 'strongreport' (iba\app\lib\strongreport.py) has zero cfg.setting()/cfg.enum() call sites — confirm this is a legitimate zero (a helper whose caller resolves config for it) or a real completeness gap
...
| escalation_type | prompted, interactive, report-stop |
