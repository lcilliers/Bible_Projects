...
| generated_at | 2026-07-30T06:11:53Z |
...
1. cfg_utility 'lexiconparse' (iba\app\lib\lexiconparse.py) has zero Cfg-method call sites (.setting()/.enum()/.tables()/... under any variable name) — confirm this is a legitimate zero (mark `cfg_utility.config_exempt=1` via `configmaint.propose`) or a real completeness gap
...
**23** registered module(s) — **11** declared `config_exempt` (a legitimate zero for config-setting/enum usage, not a completeness gap), **0** inactive (module removed/merged). See §0 "Low config-density utilities" for any NON-exempt module still flagged.
...
| cfgquality | iba/app/lib/cfgquality.py | cfgquality.py — shared config-quality checks, used by BOTH handlers/configmaint.py (the | ✓ |  |  |
