...
| generated_at | 2026-08-02T12:02:55Z |
...
1. cfg_setting 'passage.debate_session_chapter_guideline' (key not found together with a cfg.setting(...) call in any one file)
...
2. GOVERNANCE.md was last modified 2026-07-30T10:56:33Z, before the newest applied cfg_change_detail row (2026-08-02T12:02:55Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)
...
work package `book-narrative-generate` → `iba/app/ps/BookNarrative-Generate.ps1` (chained=0)
- on completion: _Narrative written — run BookNarrative-Validate.ps1 on it next._
- next-step hint: _iba\app\ps\BookNarrative-Validate.ps1 -Path <the narrative file>_
- paused override: _Estimated cost/token count printed above — answer the escalation (Approve/Reject/Revise) then re-run this exact command to make the live API call._
