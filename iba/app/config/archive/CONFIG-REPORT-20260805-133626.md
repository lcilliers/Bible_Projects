...
| generated_at | 2026-08-05T12:03:58Z |
...
7. GOVERNANCE.md was last modified 2026-08-02T13:46:47Z, before the newest applied cfg_change_detail row (2026-08-05T12:02:39Z) — check whether that change needs an entry (GOVERNANCE.md §8's own rule)
...
| spanreading | iba/app/lib/spanreading.py | spanreading.py — the `span_reading` engine: T1-T3 of the verse-lexical technique | ✓ |  |  |
...
| report | report.span_reading_output_pattern | {book}-{range}-span-reading.md | filename pattern for report.span_reading ({book}/{range} substituted) — reuses report.verse_analysis_output_dir for the base folder, same as report.verse_span_meaning did |
...
**verse-span-reading** — runs over `book` · script `iba/app/ps/VerseSpanReading.ps1`
| # | step | handler | scope | does |
| --- | --- | --- | --- | --- |
| 0 | span_reading.build | iba.app.handlers.spanreading:build | book | verse : span_reading extract — mechanical T1-T3 engine: for every code in a span's (possibly compound) strong_variant, classifies role (content lexical entry vs. grammatical formative), stem/voice-selects the operative sense from strong_meaning_parsed via morph_code (not the whole six-stem/voice paradigm), and flags — never resolves — the sibling/base-fallback ambiguity case versespanmeaningreport.meaning_for_code already detects. Runs independent of T4-T9/passage_debate. Version-aware: soft-deletes the superseded row and inserts fresh on rewrite, same convention every other iba.db table already uses. Replaces report.verse_span_meaning (retired, see BUILD.md). |
| 1 | report.span_reading | iba.app.handlers.reports:span_reading_report | book | on-demand MD report, generated from span_reading (never an independent write) — EV verse text and the resolved lexical reading placed together, per book/passage range. Requires span_reading.build to have already run for this exact range. |
...
| report.span_reading | no-readings | report-stop | no span_reading rows exist yet for this exact book/range — run span_reading.build first (it is ordinal 0 of this same work package, run automatically before this step unless called standalone). |
...
| span_reading.build | unreachable | report-stop | STEP is down and step.required_for_runs is true — cannot resolve any content-role code's sense without it. |
...
| span_reading.build | span_reading |
...
| role | TEXT |  | ✓ |  |  | 'content' (a real lexical entry) or 'function' (a grammatical formative expected to carry no independent sense, by design — see classify_role) | migration/bootstrap_span_reading.py |
| status | TEXT |  | ✓ |  |  | 'content_resolved' (sense pulled + stem/voice-selected), 'content_unregistered' (role=content but strong has no row yet — a genuine coverage gap, distinct from 'function'), or 'function' (role=function, resolved_sense correctly NULL, not a gap) | migration/bootstrap_span_reading.py |
| resolved_sense | TEXT |  |  |  |  | stem/voice-selected sense text for content_resolved rows; NULL for function/content_unregistered rows by design, never a hedge — see feedback_no_hedge_pointers_in_complete_records | migration/bootstrap_span_reading.py |
...
### `report.span_reading`
**{book} {range} — verse : span_reading extract** — output `md` · naming `stable` · archived to `archive/` · ToC on
work package `verse-span-reading` → `iba/app/ps/VerseSpanReading.ps1` (chained=1)
- on completion: _span_reading built and rendered for '{book}'._
...
| condition | path | route | message |
| --- | --- | --- | --- |
| no-readings | report-stop | terminal | no span_reading rows exist yet for this exact book/range — run span_reading.build first (it is ordinal 0 of this same work package, run automatically before this step unless called standalone). |
