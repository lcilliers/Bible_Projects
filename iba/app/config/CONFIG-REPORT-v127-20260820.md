...
| generated_at | 2026-08-20T11:39:53Z |
...
3. cfg_enum group 'escalation_next_action_dispatcher' (not looked up by name at runtime — no cfg.enum('escalation_next_action_dispatcher') or cfg_enum WHERE name='escalation_next_action_dispatcher' call site)
4. cfg_enum group 'escalation_next_action_manual' (not looked up by name at runtime — no cfg.enum('escalation_next_action_manual') or cfg_enum WHERE name='escalation_next_action_manual' call site)
...
### escalation_history
_one row per update to an item, ever_ — Append-only, one FULL SNAPSHOT row per update to an escalation item, every column's value at that version -- never updated or deleted once written (escalation-redesign-plan-v3 §2). This is the fix for the single most critical failure the old design had: escalation #715's updates being silently overwritten with no trace. `escalation` always mirrors the latest row here by construction.
...
| short_description | TEXT |  | ✓ |  |  | snapshot of escalation.short_description at this version |  |
| context | TEXT |  |  |  |  | snapshot of escalation.context (full cumulative text) at this version |  |
| comment | TEXT |  |  |  |  | snapshot of escalation.comment (full cumulative text) at this version |  |
...
| raised_at | TEXT |  | ✓ |  |  | snapshot of escalation.raised_at (constant) |  |
