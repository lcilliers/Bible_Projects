...
.SYNOPSIS
    ...
    are). NOT part of Build-Passages.ps1's sequence (deliberately — see handlers/passage.py:
    validate's docstring): run this on demand, not on every book build.
...
$json = python -m iba.app.run passage-quality --step passage.validate --run-id $runId
$code = $LASTEXITCODE
