...
> Generated 2026-07-22T06:12:24Z by `candidate.validate`. Read-only findings, not a gate. Covers the stamp (`span_candidate`), the seed decision (`candidate_seed`), and the independent substrate (`lemma_inventory`) — one worklist, not three separate checks.

- `span_candidate.candidate_tag` null: **15541** row(s)
- `candidate_seed.tag` null (candidate rows only): **0** row(s)
- `lemma_key` with no `strong` row: **46003** row(s) across 386 lemma(s)

## span_candidate.candidate_tag (the stamp)

**35945/72381** row(s) violate `pattern:candidate.tag_clean_pattern`, by category (of the samples shown below): parenthetical 18404, colon (dual-gloss) 7567, other 6360, slash (alt-gloss) 3614

| value | rows |
|---|---:|
| "'God has forgotten, he won't see'" | 5309 |
| 'practice (asah)' | 2628 |
| 'come before (bo)' | 2488 |
| 'walk (halak)' | 1501 |
| 'see (raah)' | 1300 |
| 'enemies speak lies (v11)' | 1142 |
| 'to hear: hear' | 1130 |
| 'dwell in unity (yashab)' | 1078 |
| 'return (shuv)' | 1053 |
| 'stretch out the hands (shalach)' | 843 |
| 'eat up / devour (akal - eat up my people)' | 810 |
| 'to call: call to' | 733 |
| 'bad: harmful' | 665 |
| 'lift up (nasa)' | 641 |
| 'make (sim)' | 578 |
| 'to keep: obey' | 464 |
| 'suffer (matsa)' | 455 |
| 'forever: enduring' | 438 |
| 'justice: judgement' | 422 |
| 'to perceive: understand' | 396 |
| 'spirit/breath: spirit' | 379 |
| 'I confess my iniquity, I am sorry for my sin' | 371 |
| 'bloodthirsty / men of blood (dam - save me from bloodthirsty men)' | 360 |
| 'turn aside (sur, negated)' | 300 |
| 'to strengthen: strengthen' | 291 |
| 'face: anger' | 276 |
| 'strength: soldiers' | 246 |
| 'to trust (in)' | 243 |
| 'iniquity: crime' | 233 |
| 'the law in his heart, steps not slipping' | 219 |
| 'forsake (azab)' | 215 |
| 'turn aside (natah)' | 214 |
| 'to love: lover' | 211 |
| 'to will/desire' | 206 |
| 'pass away (kalah)' | 204 |
| 'defend / judge for (shaphat - may he defend the cause of the poor)' | 202 |
| 'prevail (yakol, negated)' | 194 |
| 'extol (rum)' | 190 |
| 'swear (shaba)' | 186 |
| 'pray (shaal)' | 173 |
| 'to consecrate: consecate' | 171 |
| 'fight / attack (lacham - many attack me)' | 168 |
| 'kill (harag)' | 167 |
| 'declare (saphar)' | 151 |
| 'to call: call' | 147 |
| 'pursue (radaph)' | 143 |
| 'profane / violate (chalal - he violated his covenant)' | 142 |
| 'voice/sound: voice' | 140 |
| 'sacrifice (zabach)' | 134 |
| 'be at rest / settle (shakan - and be at rest)' | 129 |
| 'truth: faithful' | 127 |
| 'cast / throw (shalak - cast your burden on the LORD)' | 125 |
| 'to devise: design' | 123 |
| 'upright:right' | 119 |
| 'pour out (shaphak)' | 115 |
| 'to honor: honour' | 113 |
| 'to plead/comfort' | 109 |
| 'be weak: weak' | 108 |
| 'to be sorry: comfort' | 108 |
| 'to redeem: redeem' | 103 |
| 'run (ruts)' | 101 |
| 'stir up strife / band together (gur - they stir up strife)' | 93 |
| 'unfeeling (tapash)' | 92 |
| 'abide (lun)' | 84 |
| 'learn (lamad)' | 84 |
| 'be hurt (anah)' | 82 |
| 'curse (qalal)' | 82 |
| 'hide oneself (sathar - then I could hide from him)' | 81 |
| 'set (shith)' | 81 |
...
| 'vain (shav)' | 53 |
| 'evil/harm: evil' | 50 |
| 'lean / be sustained (samak - upon you I have leaned from before my birth)' | 48 |
| 'murder (ratsach)' | 47 |
| 'guilt [offering]' | 46 |
...
| 'flatter / deceive (pathah)' | 27 |
| 'be weary/toil' | 26 |
| 'come (qadam)' | 26 |
| 'fool[ish]' | 26 |
| 'offer freely (nedabah)' | 26 |
| "'with our tongue we will prevail'" | 25 |
| 'consecration: Nazirite vow' | 25 |
| 'fly away (uph - I would fly away)' | 25 |
| 'knees (berek)' | 25 |
| 'I told the glad news, I did not restrain my lips' | 24 |
| 'judge / govern (din - may he judge your people with righteousness)' | 24 |
| 'weakness: weak' | 24 |
| 'wrongdoings (alilah)' | 24 |
| 'search out (chaphas - a diligent search)' | 23 |
...
| 'to remain/endure' | 17 |
| 'laughter (sechoq)' | 15 |
| 'look away, that I may smile again' | 15 |
| 'robber/rebel' | 15 |
| 'exchange (mur)' | 14 |
...
| 'long/distant' | 13 |
| 'barren woman (aqar)' | 12 |
| 'change (chaliphah - they do not change)' | 12 |
...
| 'to die/be dead' | 10 |
| 'to give/deliver' | 10 |
| 'I was mute, and my distress grew worse' | 9 |
| 'be astounded / amazed (tamah)' | 9 |
| 'delay (mahah, negated)' | 9 |
...
| 'accusers (satan)' | 6 |
| 'ark: covenant' | 6 |
| 'be false / deal falsely (shaqar - to your covenant, negated)' | 6 |
| 'longing (shaqaq)' | 6 |
| 'mind/thought' | 6 |
| 'to ponder/confer' | 6 |
...
| 'to know/choose' | 5 |
| 'be restless / roam in distress (rud - I am restless)' | 4 |
| 'go astray (shagag)' | 4 |
