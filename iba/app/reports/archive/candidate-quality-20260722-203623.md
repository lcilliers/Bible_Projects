...
> Generated 2026-07-22T18:34:29Z by `candidate.validate`. Read-only findings, not a gate. Covers the stamp (`span_candidate`), the seed decision (`candidate_seed`), and the independent substrate (`lemma_inventory`) — one worklist, not three separate checks.
