"""escalation_redesign_v1_20260819.py — ONE-OFF: the escalation full redesign instructed by the
researcher across three review rounds of `iba/docs/escalation-redesign-plan-v3-20260819.md`
(full design record; supersedes the 2026-08-16 reset, `escalation_reset_v1_20260816.py`).

**Clean-cutover approach, per the researcher's explicit instruction (2026-08-19)** — not an
in-place migration of the 722 live rows:

    "we can create new escalations for all 4 outstanding items, and build it from there. I do not
    want to finish those with the old system. we can leave those for as is... mark the old table
    as inactive and rename the table to escalations_old so the old data can be isolated
    completely."

**What this does, in order:**

1. Safety: the full pre-change `escalation` table (722 rows) was already snapshotted to
   `iba/app/reports/archive/escalation-table-snapshot-pre-redesign-20260819.json` before this
   script ran — not redone here, just noted.
2. Renames the live `escalation` table to `escalations_old` (all 722 rows preserved, untouched) and
   re-registers it in `cfg_table`/`cfg_column` as historical/`inactive=1` — frozen, excluded from
   all new validation, never written to again.
3. Builds two new tables from `cfg_column`/`table_ddl()` (config-driven, not hand-written DDL, same
   convention as `build_data_tables()`/`retrofit_debate_lexicon_tables.py`):
   - `escalation` — current-state row per item, per plan v3 §3/§8 column set (+ `version`,
     `originator`, replacing `answered_by`).
   - `escalation_history` — append-only, one FULL SNAPSHOT row per update, per plan v3 §2 (not a
     partial diff — every column's value at that version, unique on `(escalation_id, version)`).
4. Seeds `sqlite_sequence` so new `escalation.id` values continue from 735 — the old table's ids
   (up to 734) stay unambiguous references in existing docs/commits/BUILD.md, never reused.
5. Updates `cfg_enum`: `escalation_state` gains `supersede` and `re-assigned` (replacing `re-assign`,
   which is deactivated — non-destructively, same convention as the 2026-08-16 reset);
   `escalation_next_action` is replaced wholesale: `ready_for_approval | approved | reject | revise
   | noted | review` (old `approve`/`hold` deactivated — `hold` is now `state=on-hold` only, no
   longer also an action, per the researcher's plan v2 §5 correction).
6. Re-raises the 4 genuinely open items from the old table as fresh rows in the new `escalation` +
   `escalation_history` (version 1 each), per the researcher's explicit instruction — NOT migrated
   in place, written fresh, each carrying a `related_activity` pointer back to its
   `escalations_old` id for traceability. The old rows are left exactly as they were, un-answered,
   in `escalations_old` — dead ends by design, not defects.

**What this does NOT do** (separate build steps, tracked in `iba/docs/escalation-redesign-plan-v3-
20260819.md` §8/§12 and `BUILD.md`): the rewritten `lib/escalation.py` (`raise_new`/`update`), the
rewritten `Escalation.ps1`, or updating `run.py`'s 3 direct writes + the `module_blocking` dispatch
query + the 7+ handlers that read the old function shapes — those still target the column/table
shape this migration replaces and are updated in the following commits, in that order, each
verified before the next.

    python -m iba.app.migration.escalation_redesign_v1_20260819
"""
...
OLD_MAX_ID = 734  # confirmed live immediately before this script ran

# ── new `escalation` columns (id/version auto; the rest per plan v3 §3/§8) ──────────────────────
ESCALATION_COLUMNS = [
    # name, ordinal, type, is_pk, notnull, is_unique, dflt, fk, use, expectation, filled_by
    ("id", 0, "INTEGER", 1, 1, 0, None, None,
     "serial PK, 4-digit display; continues from escalations_old's max (734) so ids stay "
    ...
    ("context", 6, "TEXT", 0, 0, 0, None, None,
    ...
    ("next_action", 10, "TEXT", 0, 0, 0, None, None,
     "what's expected of the current reader (incoming) / what the next reader should do (outgoing) "
     "-- plan v3 §1's incoming/outgoing framing.", "enum.escalation_next_action",
     "escalation.raise_new/update"),
    ("next_action_assigned_to", 11, "TEXT", 0, 0, 0, None, None, "Claude | Researcher",
     "enum.escalation_assignee", "escalation.raise_new/update"),
    ("originator", 12, "TEXT", 0, 0, 0, None, None,
    ...
    ("related_activity", 14, "TEXT", 0, 0, 0, None, None,
     ...
     "escalation.raise_new/update"),
    ("raised_at", 15, "TEXT", 0, 1, 0, None, None, "first creation datetime -- set once, immutable",
     None, "escalation.raise_new"),
    ("answered_at", 16, "TEXT", 0, 0, 0, None, None,
     "mirrors the latest escalation_history row's timestamp", None, "escalation.update"),
]

ESCALATION_TABLE_ROW = (
    "escalation",
    "One row per item, CURRENT STATE ONLY -- the escalation redesign (2026-08-19, "
    "escalation-redesign-plan-v3). Redundant with the latest escalation_history row by "
    "construction: every write updates both, in one transaction. New ids continue from "
    "escalations_old's max (734) -- see escalation_history for the full append-only record.",
...
HISTORY_COLUMNS = [
    ...
    ("source", 3, "TEXT", 0, 1, 0, None, None, "snapshot of escalation.source at this version",
     None, None),
    ("at_step", 4, "TEXT", 0, 0, 0, None, None, "snapshot of escalation.at_step at this version",
     None, None),
    ("type", 5, "TEXT", 0, 1, 0, None, None, "snapshot of escalation.type at this version",
     None, None),
    ...
    ("tried", 9, "TEXT", 0, 0, 0, None, None, "snapshot of escalation.tried at this version",
     None, None),
    ("state", 10, "TEXT", 0, 1, 0, None, None, "snapshot of escalation.state at this version",
     None, None),
    ("next_action", 11, "TEXT", 0, 0, 0, None, None,
     "snapshot of escalation.next_action at this version", None, None),
    ("next_action_assigned_to", 12, "TEXT", 0, 0, 0, None, None,
    ...
    ("raised_at", 16, "TEXT", 0, 1, 0, None, None, "snapshot of escalation.raised_at (constant)",
     None, None),
    ("answered_at", 17, "TEXT", 0, 1, 0, None, None,
...
NEW_ESCALATION_NEXT_ACTION = ["reject", "revise", "noted", "review", "ready_for_approval", "approved"]
...
def _rename_old_table(conn: sqlite3.Connection) -> int:
    ...
    conn.execute(
        ...
        "\"use\"='Historical escalation data, frozen at the 2026-08-19 redesign cutover -- "
        "722 rows, pre-dates escalation_history entirely. Read-only reference only, excluded "
        "from all new validation/correction (researcher instruction, 2026-08-19). Superseded by "
        "escalation + escalation_history.' "
...
def _update_enums(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        "UPDATE cfg_enum SET inactive=1 WHERE name='escalation_next_action' AND value IN ('approve','hold')")
...
def _raise_carryovers(conn: sqlite3.Connection, now: str) -> list[int]:
    ...
    for old_id, source, etype, short_desc, context, comment, related, assigned_to in CARRYOVER:
        cur = conn.execute(
            "INSERT INTO escalation (version, source, at_step, type, short_description, context, "
            "comment, state, next_action, next_action_assigned_to, originator, related_activity, "
            "raised_at, answered_at) VALUES (1,?,?,?,?,?,?, 'raised','review',?,'Claude',?,?,?)",
            (source, "manual", etype, short_desc, context, comment, assigned_to, related, now, now))
        new_id = cur.lastrowid
        conn.execute(
            "INSERT INTO escalation_history (escalation_id, version, source, at_step, type, "
            "short_description, context, comment, tried, state, next_action, "
            "next_action_assigned_to, originator, resolution, related_activity, raised_at, "
            "answered_at) SELECT id, version, source, at_step, type, short_description, context, "
            "comment, tried, state, next_action, next_action_assigned_to, originator, resolution, "
            "related_activity, raised_at, answered_at FROM escalation WHERE id=?", (new_id,))
...
def run() -> None:
    ...
    try:
        ...
        print("  cfg_enum: escalation_state += supersede, re-assigned (re-assign deactivated); "
              "escalation_next_action -> ready_for_approval|approved|reject|revise|noted|review")
