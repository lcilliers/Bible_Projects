"""merge_m17_into_m16_v1_20260906.py — ONE-OFF. Escalation #1525, researcher instruction verbatim,
2026-09-06: "merge M17 into M16 and mark M17 as deleted." Found in the same member-overlap scan
that surfaced M29/M18 and M38/M45 (BUILD.md #241/#242): `M16` (Wisdom & Folly, 68 members) and
`M17` (Counsel, 16 members) share 5 members outright (31.2% of the smaller cluster).
...
and M38/M45 merges this follows the exact pattern of):

  1. For M17's live members NOT already tagged to M16: REASSIGN (UPDATE in place, not a new row)
     to M16, `rationale` appended not overwritten.
  2. For M17's live members ALREADY also tagged to M16 (the overlap strongs): the M17 row is
     simply retired (soft-deleted) — no duplicate M16 row created.
  3. M16's `description`/`gloss` absorb M17's own text (appended, not overwritten).
  4. RETIRE M17: `cluster.deleted=1`. No physical delete, per project convention.

Idempotent: checks M17's live/deleted state and no-ops if already retired.

    python -m iba.app.migration.merge_m17_into_m16_v1_20260906
"""
...
def main() -> int:
    ...
    try:
        ...
        if src is None:
            ...
            print("merge_m17_into_m16_v1_20260906:")
            for line in report:
                print(f"  - {line}")
            return 1
        if src["deleted"] == 1:
