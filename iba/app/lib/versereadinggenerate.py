...
**Catalogue linkage** (`cfg_method_rule` `answers-M0.1-M0.5-D7.7`): the system prompt embeds the
live question text for `M0.1.1`-`M0.1.3` (Name and Naming), `M0.5.1`-`M0.5.10` (Lexical and
Semantic Analysis), and `D7.7.1` (operation-anchored permeability) pulled from
...
def assemble_batch_package(ctx, cluster_code: str, verse_ids: list[int]) -> dict:
    ...
    for r in lex_rows:
        ...
        if cluster_code in roles and r["strong"]:
    ...
    meaning_by_strong = {s: _meaning_sources(conn, s) for s in sorted(cluster_member_strongs)}
    ...
    questions = conn.execute(
        ...
        "OR question_code='D7.7.1') ORDER BY question_code").fetchall()
    ...
    instructions = _instructions(cluster_code, question_texts, tag_values, rules_text,
                                 [v["verse"] for v in verses_out])
    content = json.dumps({"cluster_code": cluster_code, "verses": verses_out,
                          "meaning_sources_by_strong": meaning_by_strong}, ensure_ascii=False)
...
def _instructions(cluster_code: str, questions: list[dict], tag_values: list[str],
                  rules_text: str, verse_refs: list[str]) -> str:
    ...
    return (
        ...
        f"text, and `roles_in_verse` -- every role-bearing word in that verse (not just this "
        f"cluster's own member strongs), each carrying `cluster_codes` (the full set of M-code and "
        f"role-T-code tags that word carries) and `is_home_cluster` (true if this word belongs to "
        f"the cluster you're reading). You are also given `meaning_sources_by_strong` -- for every "
        f"strong that IS a member of this cluster, its entries from up to 3 sources "
        f"(strong_meaning_tree, strong_lexicon.lsj, strong_lexicon.mounce); read all that are "
        f"present as complementary evidence, never picking one and ignoring the others.\n\n"
        f"Method rules governing this task:\n{rules_text}\n\n"
        f"Answer these catalogue questions, for every cluster-member strong they apply to:\n"
        f"{q_text}\n\n"
        ...
        f"- Every `strong` value you write must be one of this cluster's own member strongs (the "
        f"keys of `meaning_sources_by_strong`) for M0.1/M0.5 answers; for D7.7 the observation is "
        f"about an operation word's relation to a party word, both drawn from `roles_in_verse`, and "
        f"only when an operation-tagged (role-T3) word is actually present in the verse alongside a "
        f"party-tagged word.\n"
