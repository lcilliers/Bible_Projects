...
# 2026-07-08: M64 role column, M65 char_candidate/char_candidate_tag on master (seed stamp). → 3.38.0 / 3.39.0.
# 2026-07-11: M66 verse_span_index.characteristic + .ib_char_id (read char on master + normalised-index link); ib_characteristic index columns; ib_characteristic_legacy. → 3.40.0.
#   ⚠ GOVERNANCE DEBT: M64–M66 were applied by scripts, not registered in engine/migrate.py. The constant is kept in sync here so audit_word's A2 gate passes; the migrate.py registry should be back-filled with these.
EXPECTED_SCHEMA_VERSION = "3.40.0"
